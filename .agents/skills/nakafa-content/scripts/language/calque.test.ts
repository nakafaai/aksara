...
it("rejects peta as a calque for a transformation image", () => {
  const source = [
...
it("rejects narrow Indonesian technical calques", () => {
  const source = [
...
it("rejects an unexplained regularization calque without banning the subject terms", () => {
  const source = [
...
it("rejects mechanical input and constraint calques in Indonesian", () => {
  const source = [
...
it("preserves literal floors containers and gates", () => {
  const source = [
...
it("rejects an empty claim that a term is important", () => {
  assert.deepEqual(
