...
test("rejects semicolons that learners can read", () => {
  const source = [
...
test("allows semicolons required by code math entities and MDX syntax", () => {
  const source = [
    "export const metadata = {",
...
test("rejects escaped prose semicolons and semicolon entities", () => {
  const source = [
    "A \\; B.",
    "A &semi; B.",
    "A &#59; B.",
...
test("rejects semicolon entities in Markdown alt text and titles", () => {
  const source = [
...
test("rejects visible math separators but allows LaTeX spacing", () => {
  const source = [
...
test("checks metadata strings Markdown labels titles and returned JSX", () => {
  const source = [
    "export const metadata = {",
