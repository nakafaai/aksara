...
it("rejects an abrupt command after a scenario but preserves teaching flow and exercises", () => {
  const failures = {
...
it("rejects irrelevant fiction labels but preserves a concrete model limitation", () => {
  const failures = {
...
it("rejects vague flexibility claims", () => {
  const samples = {
...
it("rejects an effective label without a stated effect", () => {
  const samples = {
...
it("rejects fundamental as an unsupported importance label", () => {
  const samples = {
...
it("rejects generic learning commands at the start of metadata descriptions", () => {
  const samples = {
