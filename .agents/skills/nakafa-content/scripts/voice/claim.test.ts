...
it("rejects surprise labels and preserves the observed contradiction", () => {
  const vague = {
...
it("rejects unsupported praise for a relationship", () => {
  const sources = [
    [
      "de",
...
it("rejects empty evaluative labels instead of judging the explanation", () => {
  const samples = {
...
it("rejects robust as an unsupported quality label", () => {
  const samples = {
...
it("rejects important as an empty heading label", () => {
  const samples = {
...
it("rejects headings softened with help", () => {
  const samples = {
...
it("rejects generic everyday application headings", () => {
  const samples = {
...
it("rejects generic application labels in headings and descriptions", () => {
  const samples = {
    de: [
...
it("rejects headings that only label examples", () => {
  const samples = {
...
it("rejects generic section container headings", () => {
  const samples = {
    de: "## Überblick\n### Konzept",
    en: "## Applications\n### Concept",
    id: "## Konsep Dasar\n### Konsep",
...
it("rejects definition questions used as headings", () => {
  const samples = {
...
it("rejects headings that only promise understanding", () => {
  const samples = {
...
it("rejects calculation headings that do not name the task", () => {
  const samples = {
