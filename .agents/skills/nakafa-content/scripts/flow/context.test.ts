...
it("rejects invented learning settings without banning real class meanings", () => {
  const failures = {
...
it("rejects curriculum narration but preserves curriculum as the subject", () => {
  const failures = {
...
it("rejects a bare Indonesian range ending but preserves explicit ranges", () => {
  assert.deepEqual(
    findLessonVoiceIssues(
      "id",
...
it("rejects the aturan rambatan calque but preserves literal propagation", () => {
  assert.deepEqual(
    findLessonVoiceIssues(
      "id",
...
it("rejects detached Indonesian discussion passives", () => {
  assert.deepEqual(
    findLessonVoiceIssues(
      "id",
...
it("rejects Indonesian editorial classification in place of the subject relationship", () => {
  const failures = [
