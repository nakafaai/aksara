...
test("checks metadata descriptions but ignores code", () => {
  const source = [
...
test("reviews an unnamed path command but preserves a named path", () => {
  assert.deepEqual(
...
test("finds known ambiguous claims and operational calques", () => {
  const samples = {
...
test("finds a proven vague antecedent across a paragraph line", () => {
...
test("rejects unqualified energy density and fuel storage claims", () => {
  const samples = {
...
test("rejects metaphors that make evidence carry a conclusion", () => {
  const samples = {
...
test("rejects stock claims about what a rule allows learners to do", () => {
  const samples = {
...
test("rejects vague claims that a subject helps explain something", () => {
  const samples = {
...
test("rejects generic claims about deeper understanding", () => {
  const samples = {
...
test("rejects generic learning payoff transitions", () => {
  const samples = {
...
test("rejects vague demonstratives before a conclusion", () => {
  const samples = {
...
test("flags section openings that refer to unnamed terms", () => {
  const vagueOpenings = {
...
test("flags abstract concepts that ask questions", () => {
  const personified = {
...
test("flags unnamed Indonesian follow-up references but preserves named commands", () => {
  const ambiguous = [
