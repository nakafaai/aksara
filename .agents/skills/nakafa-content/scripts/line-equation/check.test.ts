...
} from "#nakafa-content/line-equation/check";
...
test("rejects two-point and collinear exact lines without disabled smoothing", () => {
...
test("repairs exact series without rewriting curve data", () => {
  const source = `<LineEquation
  data={[
    {
      points: [
...
/>`;
  ...
  assert.doesNotMatch(result.source, CURVE_WITH_SMOOTH_FALSE);
  assert.deepEqual(findExactLineSmoothingIssues(result.source), []);
});

test("repairs generated, computed, and explicitly smoothed straight lines", () => {
  const source = `<LineEquation
  data={[
    {
...
/>`;
  ...
  assert.doesNotMatch(result.source, SMOOTH_TRUE);
  assert.deepEqual(findExactLineSmoothingIssues(result.source), []);
});

test("preserves explicit straight lines and intentionally sampled curves", () => {
  const source = `<LineEquation
  data={[
    {
      points: [{ x: 0, y: 0 }, { x: 2, y: 2 }],
