...
- Never point at the document's own structure. The heading already tells the
  ...
  `im folgenden Beispiel` alone. The German frame `in dieser Reihenfolge` is
  blocked only when the same sentence names a procedure, through `vor`,
  `gehen`, `gehst`, or `Schritt`, because an operand order such as
