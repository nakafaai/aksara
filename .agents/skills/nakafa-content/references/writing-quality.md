...
- Indonesian learner-facing author voice uses `aku` for a singular teacher and
  `kamu` for the learner. Do not use `saya`, `Anda`, or `kalian` for that pair.
...
- Remove labels such as `latihan fiktif`, `model fiktif`, `fictional exercise`,
  `hypothetical model`, and `fiktive Aufgabe` when they merely classify an
  authored example. Introduce the quantities, assumptions, and question
  directly. If a real-world safety or
...
- A technical translation must not hide the mechanism. Do not write that a
  ...
  state whether a filter factor approaches zero or one, which solution
  component becomes smaller, and why a small random measurement error would
  otherwise cause a large change in the fitted solution.
...
- Make every document understandable from a direct visit. Define required
  terms, abbreviations, symbols, and prerequisites on first use.
...
- Do not copy the same section sequence across different nearby lesson routes.
  This rule does not apply to locale siblings. Every localized version of one
...
- Do not make a concept ask the lesson question. Replace `mitigasi bertanya
  ...
  learner, prompt, program, or API. Answer a teaching question in the
...
- Rewrite rhetorical negation around the actual teaching point. Avoid
  ...
  `daripada`, `und nicht`, `statt`, or `stattdessen` leaves the same writing
  problem behind. A punctuation change does not fix the prose. Keep a negative
  statement when it is a fact, mathematical condition, assessed text,
  quotation, or correction the learner needs. Explain the correct idea in plain
...
- The representation rules are supported by the evidence listed in
  [MDX quality](mdx-quality.md#evidence-basis), including the What Works
