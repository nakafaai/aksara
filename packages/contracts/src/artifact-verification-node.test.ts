...
import { verifySignedContentArtifact } from "./artifact-verification-node.js";
import {
...
import { createRendererManifest } from "./renderer-node.js";
import {
...
describe("server-only artifact verification", () => {
  ...
  it("rejects tampered metadata, payload, and artifact hash values", async () => {
    const artifact = signArtifact();
    const tamperedMetadata = CompiledContentPayloadSchema.make({
      ...artifact.payload,
      metadata: { ...artifact.payload.metadata, title: "Tampered" },
    });
    ...
    const metadataError = await reject(
      request({ ...artifact, payload: tamperedMetadata })
    );
    ...
    expect(metadataError._tag).toBe("ArtifactHashMismatchError");
