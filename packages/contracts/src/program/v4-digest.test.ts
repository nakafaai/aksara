...
import {
  curriculumV4Rows,
  makeProgramV4TestRecords,
  programV4CatalogRows,
} from "#contracts/test/program-v4";
        ...
        get(target, property, receiver) {
          if (property === "update") {
            return (data: BinaryLike) => {
              if (String(data).startsWith("nakafa.aksara.program-rows.v4\n")) {
                aggregate = true;
...
let records: readonly ProgramSnapshotV4Row[];

beforeAll(async () => {
  records = await Effect.runPromise(makeProgramV4TestRecords());
}, 30_000);

/** Returns one typed current program digest failure. */
function reject(
  rows: readonly ProgramSnapshotV4Row[],
...
describe("program v4 aggregate digest", () => {
  it("authenticates exact active-locale program and route closure", async () => {
    ...
    const firstProgram = records.find((record) => record.kind === "program-v4");
    if (firstProgram?.kind !== "program-v4") {
  ...
  it("rejects wrong locale sets, row integrity, order, keys, and slugs", async () => {
    const programs = programV4CatalogRows(records);
    const curricula = curriculumV4Rows(records);
    ...
    const duplicateKey = await Effect.runPromise(
      makeProgramSnapshotV4Row({ ...second.row, key: first.row.key })
    );
    const duplicateSlugRow = Schema.decodeUnknownSync(LearningProgramV4Schema)({
      ...second.row,
    ...
    const duplicateSlug = await Effect.runPromise(
      makeProgramSnapshotV4Row(duplicateSlugRow)
    );
  ...
  it("rejects route ownership, ancestry, roots, and node conflicts", async () => {
    ...
    const germanRoute = Schema.decodeUnknownSync(CurriculumRouteV4Schema)({
      ...firstRoot.row,
      locale: "de",
    ...
    const inactiveLocale = await Effect.runPromise(
      makeCurriculumSnapshotV4Row(germanRoute)
    );
    const nonCurriculum = await Effect.runPromise(
      makeProgramSnapshotV4Row({
        ...firstProgram.row,
