...
} from "#contracts/adoption/runtime";
import type { RendererManifestEnvelope } from "#contracts/adoption/schema";
import {
...
const verifyProtectedItem = Effect.fn(
  ...
  if (liveRenderer.hash !== bundle.payload.rendererManifestHash) {
    yield* verifyContentRendererCompatibility({
      payload: yield* selectVerifiedArtifactRenderer(artifact),
