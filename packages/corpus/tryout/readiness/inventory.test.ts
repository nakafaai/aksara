...
describe("assessment question readiness", () => {
  ...
  it.effect(
    "rejects incomplete inventory and blueprint coverage",
    () =>
      Effect.gen(function* () {
        const { questions, readiness, source } = yield* loadTkaReadiness();
  ...
  it.effect(
    "rejects unknown and underrepresented official dimensions",
    () =>
      Effect.gen(function* () {
        const { questions, readiness, source } = yield* loadTkaReadiness();
  ...
  it.effect(
    "rejects a false topic owner, response mix, or group count",
    () =>
      Effect.gen(function* () {
        const { questions, readiness, source } = yield* loadTkaReadiness();
