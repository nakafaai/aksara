...
These public interfaces were verified live against Nakafa commit [`927d7f681a43acb205d8abcbd63101a8ecd31b41`](https://github.com/nakafaai/nakafa.com/commit/927d7f681a43acb205d8abcbd63101a8ecd31b41): the REST index, OpenAPI contract, and MCP endpoint return the same release identity, and `@nakafa/cli@0.1.1` is available from npm with verifiable provenance.

At that revision, [`api.ts`](https://github.com/nakafaai/nakafa.com/blob/927d7f681a43acb205d8abcbd63101a8ecd31b41/packages/backend/convex/routes/agent/api.ts) owns REST, [`route.ts`](https://github.com/nakafaai/nakafa.com/blob/927d7f681a43acb205d8abcbd63101a8ecd31b41/packages/backend/convex/routes/agent/mcp/route.ts) owns MCP, [`document.ts`](https://github.com/nakafaai/nakafa.com/blob/927d7f681a43acb205d8abcbd63101a8ecd31b41/packages/backend/agent/openapi/document.ts) generates OpenAPI, and [`program.ts`](https://github.com/nakafaai/nakafa.com/blob/927d7f681a43acb205d8abcbd63101a8ecd31b41/packages/cli/src/program.ts) owns the CLI command surface.
...
Search returns 10 results by default and accepts at most 10 per request. The search window contains at most 10 results, with `offset` from 0 to 9. When using a smaller `limit`, follow `has_more` and `next_offset` to read the next page within that window. Clients should budget for no more than 120 requests per 60 seconds from one IP and handle HTTP 429 responses with backoff.
