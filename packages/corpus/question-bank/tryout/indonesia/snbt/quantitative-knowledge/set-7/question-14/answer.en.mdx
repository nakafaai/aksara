...
In <InlineMath math="g(x) = 2^x - 1" />, the variable <InlineMath math="x" /> appears in the exponent. This makes <InlineMath math="g" /> an exponential function. A linear function has the form <InlineMath math="y = mx + c" />, so statement <InlineMath math="(1)" /> is **incorrect**.
