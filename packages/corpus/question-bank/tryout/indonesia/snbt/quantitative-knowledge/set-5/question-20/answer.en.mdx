...
Thus, we give both fractions a **common denominator**, which is <InlineMath math="x^2 - 4" /> or <InlineMath math="(x - 2)(x + 2)" />:
...
So, the simplified result is <InlineMath math="\frac{x - 4}{x^2 - 4}" />, with the original domain restriction <InlineMath math="x \neq -2, 2" />.
