export const metadata = {
  dateModified: "2026-09-12",
  title: "Explanation for Question 5",
  authors: [{ name: "Nabil Akbarazzima Fatih" }],
  datePublished: "2025-11-26",
};

Start by determining the equation of the parabola from the graph. The **vertex** is at <InlineMath math="(4, 1)" />.

The general form of a **horizontal parabola equation** with vertex <InlineMath math="(x_p, y_p)" /> is <InlineMath math="x = a(y - y_p)^2 + x_p" />.

Substitute <InlineMath math="x_p = 4" /> and <InlineMath math="y_p = 1" />:

<BlockMath math="x = a(y - 1)^2 + 4" />

The curve passes through the point <InlineMath math="(0, -1)" />. We substitute this point to find the value of <InlineMath math="a" />.

<BlockMath math="\begin{aligned}
0 &= a(-1 - 1)^2 + 4 \\
0 &= 4a + 4 \\
a &= -1
\end{aligned}" />

So, the equation of the curve is <InlineMath math="x = -(y - 1)^2 + 4" />.

This means statement (2) is true, because it gives the same equation in function notation, <InlineMath math="f(y) = -(y - 1)^2 + 4" />.

#### Vertical axis intersections

On the **vertical axis**, set <InlineMath math="x = 0" />.

<BlockMath math="\begin{aligned}
0 &= -(y - 1)^2 + 4 \\
(y - 1)^2 &= 4 \\
y - 1 &= \pm 2
\end{aligned}" />

From <InlineMath math="y - 1 = \pm 2" />, we get <InlineMath math="y = 3" /> or <InlineMath math="y = -1" />. The intersection points are <InlineMath math="(0, 3)" /> and <InlineMath math="(0, -1)" />.

Statement (1) gives <InlineMath math="(0, 2)" />, so it is false.

#### Horizontal axis intersection

On the **horizontal axis**, <InlineMath math="y = 0" />.

<BlockMath math="\begin{aligned}
x &= -(0 - 1)^2 + 4 \\
x &= -1 + 4 \\
x &= 3
\end{aligned}" />

The intersection point with the <InlineMath math="x" />-axis is <InlineMath math="(3, 0)" />, so statement (3) is false because it states <InlineMath math="(1, 0)" />.

Finally, the vertex is <InlineMath math="(4, 1)" />, so its abscissa, or <InlineMath math="x" /> value, is <InlineMath math="4" />. Therefore statement (4) is true.

Therefore, the correct statements are (2) and (4).
