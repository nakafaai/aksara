export const metadata = {
  title: "Solution for Question 7",
  authors: [{ name: "Nabil Akbarazzima Fatih" }],
  datePublished: "2025-12-23",
  dateModified: "2026-09-07",
};

We are given the following **determinant equation**:

<BlockMath math="\begin{gathered}\begin{vmatrix}3x&x\\2&x\end{vmatrix}\\=\begin{vmatrix}3&1\\1&2\end{vmatrix}\end{gathered}" />

For a <InlineMath math="2\times2" /> matrix, use the determinant rule:

<BlockMath math="\begin{vmatrix}a&b\\c&d\end{vmatrix}=ad-bc" />

Determinant of the left side:

<BlockMath math="\begin{gathered}\begin{vmatrix}3x&x\\2&x\end{vmatrix}\\=(3x)(x)-(x)(2)\\=3x^2-2x\end{gathered}" />

Determinant of the right side:

<BlockMath math="\begin{gathered}\begin{vmatrix}3&1\\1&2\end{vmatrix}\\=(3)(2)-(1)(1)\\=6-1=5\end{gathered}" />

Now we equate both results:

<BlockMath math="3x^2 - 2x = 5" />

Move <InlineMath math="5" /> to the left side to form a quadratic equation:

<BlockMath math="3x^2 - 2x - 5 = 0" />

Use **factoring by grouping**. We look for two numbers that multiply to <InlineMath math="3 \times (-5) = -15" /> and sum to <InlineMath math="-2" />. These numbers are <InlineMath math="-5" /> and <InlineMath math="3" />.

<MathContainer>
  <BlockMath math="3x^2 - 5x + 3x - 5 = 0" />
  <BlockMath math="x(3x - 5) + 1(3x - 5) = 0" />
  <BlockMath math="(x + 1)(3x - 5) = 0" />
</MathContainer>

Thus, the solutions are:

<MathContainer>
  <BlockMath math="x + 1 = 0 \Rightarrow x = -1" />
  <BlockMath math="3x - 5 = 0 \Rightarrow x = \frac{5}{3}" />
</MathContainer>

So, the values of <InlineMath math="x" /> that satisfy the equation are <InlineMath math="\frac{5}{3}" /> and <InlineMath math="-1" />.
