...
Cells can increase their number by dividing. Viruses need a suitable host cell to produce new virions, because they carry no machinery for building proteins or copying genetic material on their own. The sections below describe what a virion is and how the replication cycle proceeds.
...
The lytic cycle follows an ordered sequence from cell recognition to the release of newly assembled virions. Each step describes a different event. Reordering the steps breaks the cycle, so the order itself is part of what the section teaches.
