...
<Highlight>Choose each prevention measure from the virus's transmission route.</Highlight> An action that interrupts one route may have little effect on another. A measure works when it blocks the step the virus actually uses to move between people.
...
Viruses can spread through contact, droplets, air, blood or body fluids, and other [route](https://www.cdc.gov/infection-control/hcp/isolation-precautions/scientific-review.html)s. <Highlight>The route determines which measure can interrupt transmission</Highlight>.
