...
While a program is running, <Highlight>every Python value is an object</Highlight>. Numbers, functions, classes, and modules each have a type, an identity that distinguishes the object, and a stored value.
...
The same object model supports several ways to organize a program. A procedural style sequences steps and passes data between them, a functional style composes a calculation from functions, and an object-oriented style **keeps data together with the operations** that use it. Knowing which style a file follows makes its flow easier to read.
...
The Python version keeps the parking rules and operations in one class. The `occupy` method checks the remaining capacity whenever it is called, but outside code can still change the public `max` and `occ` attributes directly. This short example therefore does not fully protect the object's state.

### Functional Style

A pure function computes **a result from inputs without changing state**. Here, capacity and the vehicle count are arguments, and the function returns the number of free spaces.
...
The statement that everything is an object means Python treats every runtime value through one object model, so a number, a string, and a list are all instances of some class. What differs is the set of attributes and methods that class provides, because <Highlight>the type decides the available tools</Highlight> rather than the object model itself.

### Numbers are Objects

An integer is an **object with methods**. The example calls `bit_length()` on `42` and prints the result.
...
print(dir(result))          # Explore attribute names`
  }]}
/>

### Strings are Objects

A string is an object with methods that **build new values**. The example calls several string methods on one message.
...
A list provides **methods that change its contents**, such as `append()`, alongside read-only methods such as `count()`. The example starts from a fruit list and calls the list methods.
...
A dictionary is an object with methods for **reading and updating** its pairs. The example uses a student record to call those methods.
...
You can **pass functions as arguments**, store them in containers, and assign them to variables like other values.
...
class Vector2D:
    ...
    def __bool__(self):
        """Return False for the zero vector and True for every other vector."""
...
| Stage | First book | Alice’s loans |
| --- | --- | --- |
| Before borrowing | Available, no borrower | Empty |
| After `borrow_book()` | Borrowed by Alice | Contains the first book |
| After `return_book()` | Available again | Empty |

The book titles, authors, and identifiers are sample data. Book and member lists remain public attributes. The example teaches object relationships rather than multiuser storage.
