...
Versi Python mengelompokkan aturan dan operasi parkir dalam satu class. Method memeriksa input, tetapi atribut seperti `vehicles` masih dapat diubah dari luar. Jika state harus dijaga, sediakan interface yang diperlukan dan jangan mengekspos objek mutable tersebut secara langsung.

## Sistem Parkir sebagai Contoh

Pada contoh ini, sistem parkir menjadi objek dengan state dan behavior. State menyimpan kapasitas maksimum dan jumlah tempat yang terisi. Method menerima mobil masuk atau mencatat mobil keluar.
