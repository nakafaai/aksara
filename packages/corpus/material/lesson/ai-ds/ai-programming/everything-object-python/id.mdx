...
Saat program berjalan, <Highlight>setiap nilai di Python merupakan objek</Highlight>. Bilangan, fungsi, class, dan modul memiliki tipe, identitas yang membedakannya dari objek lain, serta nilai yang disimpan.
...
Model objek yang sama mendukung beberapa cara menyusun program. Gaya procedural menyusun langkah dan aliran data secara berurutan, gaya fungsional menyusun komputasi dari fungsi yang dipanggil berlapis, dan gaya berorientasi objek **menyimpan data bersama operasi** yang mengolahnya. Mengenali ketiganya membantumu menebak arah alur program saat membaca kode orang lain.
...
Fungsi murni menghitung **hasil dari input tanpa mengubah state**. Pada versi ini, kapasitas dan jumlah kendaraan diberikan sebagai argumen, lalu fungsi mengembalikan jumlah tempat kosong.

<CodeBlock data={[{ language: "python", filename: "functional_parking.py", code: `def available_spaces(capacity, occupied):
    return capacity - occupied

print(available_spaces(100, 1))
# Output: 99` }]} />
...
"Setiap nilai adalah objek" berarti Python memakai satu model objek untuk seluruh nilai saat program berjalan. Atribut dan method yang tersedia tetap bergantung pada <Highlight>tipe objek</Highlight> tersebut.

### Angka adalah Objek

Bilangan bulat adalah **objek yang punya method**. Contoh berikut memanggil `bit_length()` pada `42` lalu mencetak hasilnya.
...
print(dir(result))          # Menampilkan nama atribut untuk eksplorasi`
  }]}
/>

### String adalah Objek

String adalah objek dengan method yang **menghasilkan nilai baru**. Contoh berikut memanggil beberapa method string pada satu teks.
...
List menyediakan **method yang mengubah isi** seperti `append()`, serta method pembacaan seperti `count()` yang tidak mengubah list. Contoh berikut berawal dari daftar buah lalu memanggil method list.
...
Dictionary adalah objek dengan method untuk **membaca dan memperbarui** pasangannya. Contoh berikut memakai data seorang mahasiswa untuk memanggil method tersebut.
...
Fungsi dapat **diberikan sebagai argumen**, disimpan di dalam collection, dan ditempatkan pada variabel seperti nilai lain.
...
| Tahap | Buku pertama | Pinjaman Alice |
| --- | --- | --- |
| Sebelum peminjaman | Tersedia, tanpa peminjam | Kosong |
| Setelah `borrow_book()` | Dipinjam oleh Alice | Memuat buku pertama |
| Setelah `return_book()` | Tersedia kembali | Kosong |

Judul, penulis, dan kode buku pada contoh ini adalah data latihan. Daftar buku dan anggota tetap berupa atribut publik. Contoh mengajarkan hubungan objek, bukan penyimpanan multiuser.
