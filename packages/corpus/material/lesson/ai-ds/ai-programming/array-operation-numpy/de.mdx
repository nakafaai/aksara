...
[Broadcasting](https://numpy.org/doc/stable/user/basics.broadcasting.html) ist NumPys Regel zum Verknüpfen von Arrays mit **unterschiedlichen, aber kompatiblen Shapes**. Eine Dimension der Größe eins wird so behandelt, als hätte sie dieselbe Größe wie die entsprechende Dimension des anderen Arrays, ohne die Daten in ein größeres Array zu kopieren.
...
Zwei Arrays lassen sich ohne ausdrückliche Schleife verbinden, wenn NumPy ihre Dimensionen <Highlight>selbst ausrichten</Highlight> kann. Um zu entscheiden, ob zwei Shapes kompatibel sind, vergleicht NumPy ihre Dimensionen von rechts nach links und stoppt beim ersten Paar, das sich nicht ausrichten lässt.

Drei Regeln decken jeden Fall ab. Die Tabelle unten nennt für jede Regel die geprüfte Bedingung und das Ergebnis, das NumPy dafür liefert.

| Regel | Bedingung an die Shapes | Was NumPy tut |
| --- | --- | --- |
| Regel <InlineMath math="1" /> | Die beiden Shapes haben eine unterschiedliche Anzahl an Dimensionen | NumPy ergänzt links Dimensionen der Größe <InlineMath math="1" /> beim kürzeren Shape |
| Regel <InlineMath math="2" /> | Ein ausgerichtetes Paar hat gleich große Seiten, oder eine Seite hat die Größe <InlineMath math="1" /> | Gleiche Größen passen bereits. Eine Dimension der Größe eins wird gedanklich wiederverwendet, ohne wiederholte Kopien anzulegen |
| Regel <InlineMath math="3" /> | Ein ausgerichtetes Paar hat unterschiedliche Größen und keine Seite hat die Größe <InlineMath math="1" /> | NumPy löst einen `ValueError` aus |

### Array mit Skalar

Broadcasting gilt auch zwischen einem **Array und einer einzelnen Zahl**. NumPy dehnt den Skalar auf jedes Element aus, deshalb brauchst du keine Schleife.
...
Bei den Shapes `(2, 3)` und `(2,)` **passen die letzten Dimensionen nicht**: Ihre Größen sind `3` und `2`, und keine ist `1`. NumPy löst vor der Addition einen `ValueError` aus.
...
Der Operator `*` multipliziert **Elemente an entsprechenden Positionen**. Für die beiden eindimensionalen Arrays oben liefert `@` das Skalarprodukt: `0 + 3 + 4 + 3 + 0` ergibt `10`. Bei zwei Matrizen gilt die Zeile-mal-Spalte-Regel. Setze `np.dot()` nicht für alle Array-Formen mit `@` gleich. Bei höheren Dimensionen unterscheiden sich ihre Regeln.
...
## Datenstandardisierung mit Z Scores
...
X = rng.standard_normal((5, 3)) * 10 + 50  # Stichprobe aus einer Normalverteilung mit Mittelwert 50 und Standardabweichung 10
...
# Z-Standardisierung durchführen
Z = (X - mu) / sigma

print("Daten nach Z-Standardisierung:")
print(Z)
