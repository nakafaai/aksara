...
[Broadcasting](https://numpy.org/doc/stable/user/basics.broadcasting.html) ist <Highlight>NumPys Regel zum Verknüpfen von Arrays mit unterschiedlichen, aber kompatiblen Shapes</Highlight>. Eine Dimension der Größe eins wird so behandelt, als hätte sie dieselbe Größe wie die entsprechende Dimension des anderen Arrays, ohne die Daten in ein größeres Array zu kopieren.
...
| Regel <InlineMath math="2" /> | Ein ausgerichtetes Paar hat gleich große Seiten, oder eine Seite hat die Größe <InlineMath math="1" /> | NumPy wiederholt den einzelnen Eintrag entlang dieser Dimension, bis beide Größen passen |
...
Nicht jedes Formpaar lässt sich ausrichten. Das folgende Beispiel zeigt ein Formpaar, das Regel <InlineMath math="3" /> verletzt, und die Fehlermeldung von NumPy.
