...
Broadcasting is NumPy's rule system for combining arrays with different but compatible shapes. It expands dimensions of size one conceptually, without copying the values into a larger array. The [NumPy broadcasting guide](https://numpy.org/doc/stable/user/basics.broadcasting.html) covers additional examples.
...
2. **Rule** <InlineMath math="2" />: Two aligned dimensions are compatible when they are equal or one of them has size <InlineMath math="1" />; a size-one dimension is virtually expanded to the other size
