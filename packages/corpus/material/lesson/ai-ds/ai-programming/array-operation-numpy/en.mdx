...
| Rule <InlineMath math="2" /> | An aligned pair has equal sizes, or one side has size <InlineMath math="1" /> | NumPy repeats the single entry along that dimension until both sizes match |
...
Not every pair of shapes can be aligned. The example below shows a shape pair that breaks rule <InlineMath math="3" /> and the error NumPy returns.
