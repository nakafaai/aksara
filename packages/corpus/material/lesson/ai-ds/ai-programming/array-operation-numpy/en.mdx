...
Broadcasting is NumPy's rule for combining arrays with different but compatible shapes. NumPy treats a dimension of size one as if its size matched the paired dimension, without copying the data into a larger array. [NumPy](https://numpy.org/doc/stable/user/basics.broadcasting.html) provides additional examples.
...
1. **Rule** <InlineMath math="1" />: If the ranks differ, treat the lower-rank shape as if dimensions of size <InlineMath math="1" /> were added on the left
...
[NumPy](https://numpy.org/doc/stable/user/basics.html) provides more details and practical examples for array operations.
