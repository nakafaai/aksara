...
[Broadcasting](https://numpy.org/doc/stable/user/basics.broadcasting.html) is NumPy's rule for combining arrays with **different but compatible shapes**. A dimension of size one is treated as if it had the same size as the corresponding dimension of the other array, without copying the data into a larger array.
...
Two arrays combine without an explicit loop when NumPy can <Highlight>line up their dimensions</Highlight> on its own. To decide whether two shapes are compatible, NumPy compares their dimensions from right to left and stops at the first pair that cannot be aligned.

Three rules cover every case. The table below states the condition each rule checks and the result NumPy produces for that condition.

| Rule | Condition on the shapes | What NumPy does |
| --- | --- | --- |
| Rule <InlineMath math="1" /> | The two shapes have a different number of dimensions | NumPy adds dimensions of size <InlineMath math="1" /> on the left of the shorter shape |
| Rule <InlineMath math="2" /> | An aligned pair has equal sizes, or one side has size <InlineMath math="1" /> | Equal sizes already match. A size-one dimension is reused conceptually without materializing repeated copies |
| Rule <InlineMath math="3" /> | An aligned pair has different sizes and neither side has size <InlineMath math="1" /> | NumPy raises a `ValueError` |

### Array with Scalar

Broadcasting also applies between an **array and a single number**. NumPy stretches the scalar across every element, so you do not need an explicit loop.
...
For shapes `(2, 3)` and `(2,)`, the **last dimensions do not match**: their sizes are `3` and `2`, and neither is `1`. NumPy raises `ValueError` before performing the addition.
...
The `*` operator multiplies **entries at the same position**. For the two one-dimensional arrays above, `@` returns their dot product: `0 + 3 + 4 + 3 + 0` equals `10`. For two matrices, `@` follows the row-by-column rule. Do not treat `np.dot()` as equivalent to `@` for all array shapes. Their rules differ in higher dimensions.
...
## Data Standardization with Z Scores
...
The z-score formula is <InlineMath math="Z = \frac{X - \mu}{\sigma}" />, where:
...
X = rng.standard_normal((5, 3)) * 10 + 50  # Samples from a normal distribution with mean 50 and std 10
...
print("Data after z-score standardization:")
print(Z)
