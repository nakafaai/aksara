...
Python provides numeric built-ins such as `abs()` and `round()` without an import. They work across types through Python's numeric protocols: a type can define the operation it supports, rather than Python selecting from several statically overloaded function declarations.
