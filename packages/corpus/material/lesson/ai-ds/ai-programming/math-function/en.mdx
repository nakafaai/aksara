...
`from math import *` can silently overwrite an existing name, makes each imported name's owner harder to trace, and lets a future module change alter the importing namespace. Explicit imports keep those dependencies reviewable.
...
Therefore `round(4.5)` produces <InlineMath math="4" />, while `round(3.5)` produces <InlineMath math="4" />. Decimal-looking float values are stored in binary, so a value that appears to be an exact tie may not be one internally. Use decimal arithmetic when a domain requires decimal rounding rules.
...
Choose an import style that makes ownership obvious at the call site and introduces only the names the module needs.

### Guide to Choosing Import Methods

1. **Prefer `import math`** when several members are used and their origin should stay visible.
...
4. **Avoid `from math import *`** because it obscures ownership and can create name conflicts.
