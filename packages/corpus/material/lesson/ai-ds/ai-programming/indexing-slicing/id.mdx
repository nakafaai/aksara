...
Python menggunakan sistem penomoran yang dimulai dari <InlineMath math="0" /> dan bukan <InlineMath math="1" />. Jadi elemen pertama memiliki indeks <InlineMath math="0" />, elemen kedua memiliki indeks <InlineMath math="1" />, dan seterusnya. Selain itu, Python juga mendukung indeks negatif yang menghitung mundur dari akhir.
...
Batas slice dipotong ke rentang sequence, sehingga `s[10:]` pada string yang lebih pendek menghasilkan `""` dan bukan `IndexError`. Indexing langsung seperti `s[10]` tetap menghasilkan error. Pilih berdasarkan apakah elemen yang tidak ada merupakan kesalahan atau subsequence kosong memang bermakna.
