...
Indexing <Highlight>selects one element of a string or list by its position</Highlight>. For example, if `letters = ["a", "b", "c"]`, then `letters[1]` returns `"b"`.
...
Omit the start or stop bound when the selection should extend to one end of the data. An empty start means the slice begins at index `0`, and an empty stop means it runs to the final element.
