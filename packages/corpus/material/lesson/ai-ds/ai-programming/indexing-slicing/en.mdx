...
Slicing takes one part of a string or list that can contain several elements. Like choosing a slice from a loaf of bread, you set where the slice starts and stops.

### Simple Slicing

Basic slicing syntax uses the format `[i:j]` where `i` is the start index and `j` is the end index. The result includes the start index and stops before the end index.
...
Comparing the slice with its `range()` model makes the start, stop, and step rules visible, especially when the step is negative.
...
These examples use indexing where a position is known and slicing where a contiguous or stepped subsequence is intended. Reversing a sequence with `[::-1]` is concise. File extensions are different: their length varies, so `Path.suffix` expresses that domain rule more reliably than a fixed slice.

Slice bounds are clipped to the sequence, so `s[10:]` on a shorter string returns `""`. Direct indexing such as `s[10]` still raises `IndexError`. Choose the form that matches whether an empty subsequence makes sense for your task.

Negative indices let you count from the end. Use `s[-1]` for the last element or `s[-3:]` for the last three elements. This approach is useful in file processing, text parsing, and dataset work when you care about a position near the end.
