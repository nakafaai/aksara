...
A slice is written as `start:stop:step`. The start index is included, the stop index is excluded, and a negative step walks the sequence from right to left. The subsections below state each rule and show the length a slice produces.
...
Omit the start or stop bound when the selection should extend to one end of the data. An empty start means the slice begins at index `0`, and an empty stop means it runs to the final element. The subsections below list the shorthand forms and the values each one produces.
