...
Python uses a numbering system that starts from <InlineMath math="0" /> rather than <InlineMath math="1" />. So the first element has index <InlineMath math="0" />, the second element has index <InlineMath math="1" />, and so on. Additionally, Python also supports negative indices that count backwards from the end.
...
An index outside the valid range raises `IndexError`. Python sequences perform this bounds check instead of returning an arbitrary value.
