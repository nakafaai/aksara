...
Python verwendet ein Nummerierungssystem, das bei <InlineMath math="0" /> beginnt und nicht bei <InlineMath math="1" />. Das erste Element hat also den Index <InlineMath math="0" />, das zweite Element hat den Index <InlineMath math="1" /> und so weiter. Darüber hinaus unterstützt Python auch negative Indizes, die vom Ende rückwärts zählen.
...
Mit Slicing wird ein Teil einer Zeichenfolge oder Liste ausgewählt, der mehrere Elemente enthalten kann. Wie bei einer Brotscheibe werden Anfang und Ende des Ausschnitts festgelegt.

### Einfaches Slicing

Die grundlegende Slicing-Syntax verwendet das Format `[i:j]`, wobei `i` der Startindex und `j` der Endindex ist. Der Endindex ist im Ergebnis nicht enthalten.
