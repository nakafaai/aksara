...
When Python executes `x = 2`, it evaluates the expression <InlineMath math="2" /> and binds the name `x` to the resulting integer object. Object allocation, reuse, and physical memory placement are implementation details rather than behavior your program should assume.
...
Assignment `y = x` guarantees that both names refer to the same object at that moment. Creating another equal value only guarantees equality rather than identity. Some Python implementations reuse immutable objects, but correct code never depends on that optimization.
