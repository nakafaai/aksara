...
Python has strict rules for variable naming that you must follow for code to run correctly. A name may contain letters, digits, and underscores, but it cannot start with a digit, and it cannot be a reserved keyword. The sections below state the rules and then the naming habits that keep code readable.
...
Python reserves certain words as part of its syntax, so they cannot be used as variable names. Assigning to one of them raises a `SyntaxError` before the program starts. The sections below name the keywords and the habit of checking a name before you use it.
...
Python identifiers must follow the syntax rules above. Within those rules, choose names that state what each value represents. A name such as `total_price` tells the next reader more than `tp`, and the same habit keeps a longer program readable months later. The sections below collect the conventions most Python code follows.
