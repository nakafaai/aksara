...
Dalam Python, variabel adalah nama yang terikat pada sebuah objek. Nama itu bukan kotak penyimpanan bertipe tetap: assignment dapat mengikatnya ke objek lain, sedangkan nama lain mungkin tetap merujuk pada objek semula.
...
Built-in `id()` mengembalikan integer yang mengidentifikasi sebuah objek secara unik selama masa hidup objek itu. CPython lazimnya menurunkannya dari alamat memori, tetapi Python tidak menjanjikan interpretasi tersebut pada setiap implementasi.
...
Assignment `y = x` menjamin kedua nama merujuk pada objek yang sama pada saat itu. Membuat nilai lain yang setara menjamin bahwa kedua nilai lolos pemeriksaan equality. Python tetap dapat membuat objek yang terpisah. Beberapa implementasi Python menggunakan ulang objek immutable, tetapi kode yang benar tidak bergantung pada optimasi tersebut.
...
Berbeda dengan bahasa yang memerlukan deklarasi variabel terpisah, Python mengikat nama saat sebuah assignment berjalan. Objek memiliki tipe, dan `type(value)` melaporkan tipe objek yang sedang terikat pada sebuah nama.
...
Identifier dimulai dengan underscore atau huruf Unicode, lalu dapat diteruskan dengan huruf identifier, underscore, dan digit seperti <InlineMath math="0,1,2,\dots,9" />. Python menerima banyak identifier non-ASCII, tetapi nama `snake_case` ASCII yang deskriptif biasanya lebih mudah diketik, dicari, dan dibagikan lintas tools.
