...
<Highlight>String Python bersifat immutable</Highlight>, sehingga isi objek string tidak dapat diedit langsung. Operasi pada string menghasilkan string baru. Variabel dapat diarahkan ke objek baru tersebut, sedangkan objek string asal tetap tidak berubah.
...
Method berikut mengembalikan string baru dan membiarkan string aslinya tetap. Hal itu terjadi karena string bersifat immutable, sehingga setiap perubahan menghasilkan nilai baru, bukan mengubah nilai lama. Subbagian berikut mengelompokkannya menurut perubahan yang dibuat.
...
Satu kelompok method mencari letak teks, kelompok lain menjawab pertanyaan ya atau tidak tentangnya. Method pencari mengembalikan posisi, sedangkan method pemeriksa mengembalikan `True` atau `False`. Subbagian berikut memisahkan keduanya.
...
Kedua method ini mengubah cara string dibagi menjadi bagian. `replace` menukar satu potongan teks dengan potongan lain, sedangkan `split` memecah string menjadi list pada pemisah tertentu. Subbagian berikut membahas penggantian lebih dulu, lalu pemisahan.
...
Karena string method mengembalikan string baru, beberapa transformasi dapat dirangkai dalam satu ekspresi. Pertahankan chain cukup pendek agar setiap langkah tetap jelas. Beri nama pada nilai antara jika ada validasi atau penjelasan di tengah proses.
