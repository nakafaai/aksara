export const metadata = {
    title: "Methoden für Strings",
  description: "Verarbeite Text mit Python-String-Methoden für Umwandlung, Suche und Zeichenklassifikation.",
...
<Highlight>Python-Strings sind immutable</Highlight>: Der Wert eines String-Objekts lässt sich nicht direkt bearbeiten. Eine Operation kann einen neuen String erzeugen und eine Variable kann anschließend auf dieses neue Objekt verweisen. Das ursprüngliche String-Objekt bleibt dabei unverändert.
...
raw_newline = r"\\n"
print(f"Raw-Schreibweise {raw_newline!r}: {len(raw_newline)} Codepoints")`
    }
  ]}
/>
...
## Methoden zur Transformation von Strings

Diese Methoden liefern einen neuen String und **lassen den ursprünglichen unverändert**. Das liegt daran, dass ein String unveränderlich ist, jede Umwandlung also einen zweiten Wert erzeugt, statt den ersten zu bearbeiten.

### Ändern der Großschreibung und Kleinschreibung
...
Ohne Argument entfernen `strip()`, `lstrip()` und `rstrip()` umgebenden Whitespace. Mit `chars` gibst du Python eine **Menge einzelner Zeichen**. Python entfernt passende Zeichen wiederholt vom jeweiligen Rand.
...
## Strings durchsuchen und Zeichen prüfen

Eine Gruppe von Methoden findet die Stelle eines Musters im Text, eine andere Gruppe beantwortet Ja-Nein-Fragen dazu. Die suchenden Methoden **geben eine Position zurück**, die prüfenden Methoden geben `True` oder `False` zurück.
...
Methoden wie `isalpha()` und `isdigit()` geben einen booleschen Wert zurück, nachdem sie geprüft haben, ob die Codepoints im String eine bestimmte Kategorienregel erfüllen. Diese Methoden prüfen **nur Zeichenkategorien**. Für Benutzernamen, E-Mail-Adressen oder Kontonummern brauchst du zusätzliche Regeln für das vollständige Format.
...
## Suchmethoden für Präfixe und Suffixe
...
Beide Methoden akzeptieren ein Tupel, sodass ein Aufruf mehrere Präfixe oder Suffixe prüfen kann.

Das Präfix `https://` ordnet den Text nach dem URL-Schema ein. Es belegt weder die Vertrauenswürdigkeit der Website noch die Gültigkeit der Adresse.

## Methoden Replace und Split

Diese beiden Methoden ändern, wie ein String in Teile zerlegt wird. `replace` **tauscht einen Teilstring** gegen einen anderen, `split` zerlegt den String an einem Trennzeichen in eine Liste.
...
## Verkettung von Stringmethoden

Transformationsmethoden, die **Strings zurückgeben**, lassen sich verketten. Eine Methode wie `split()` liefert eine Liste. Die nächste Operation muss zu diesem Ergebnistyp passen. Halte die Kette so kurz, dass jeder Schritt klar bleibt. Wenn dazwischen eine Prüfung oder Erklärung nötig ist, gib dem Zwischenergebnis einen Namen.

Nach dem ersten `.strip()` bleiben Leerzeichen zwischen `+++` und `Python`. Deshalb entfernt `.strip("+- ")` **Pluszeichen, Minuszeichen und Leerzeichen** an beiden Rändern vor der Kleinschreibung.
...
## Praktische Anwendungen von Methoden für Strings

Jedes kleine Beispiel zeigt das Verhalten einer Stringmethode unter **klar genannten Annahmen**. Validatoren und Parser für den Produktionseinsatz brauchen Regeln für ihre echten Daten.

Die Telefonfunktion formatiert nur zwei Beispielmuster. Indonesische Telefonnummern sind nicht auf diese Längen beschränkt. Dateinamen werden ohne Verzeichnispfad angenommen. Die E-Mail-Normalisierung oben zeigt Stringverarbeitung, keine vollständige Adressvalidierung.
...
def format_phone_number(phone):
    ...
    # Zwei Beispielmuster, nicht alle indonesischen Nummerierungsregeln
...
Setze jede Methode so ein, wie sie tatsächlich arbeitet. Eine Stringoperation kann einen Prüfschritt übernehmen. Für eine vollständige Validierung sind oft weitere Regeln nötig.
