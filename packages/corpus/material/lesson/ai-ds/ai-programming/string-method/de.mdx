...
Diese Methoden liefern einen neuen String und lassen den ursprünglichen unverändert. Das liegt daran, dass ein String unveränderlich ist, jede Umwandlung also einen zweiten Wert erzeugt, statt den ersten zu bearbeiten. Die Unterabschnitte unten gruppieren sie nach der Änderung, die sie vornehmen.
...
Eine Gruppe von Methoden findet Textstellen, eine andere beantwortet Ja-Nein-Fragen dazu. Die suchenden Methoden geben eine Position zurück, die prüfenden `True` oder `False`. Die Unterabschnitte unten trennen beide.
...
Diese beiden Methoden ändern, wie ein String in Teile zerlegt wird. `replace` tauscht einen Teilstring gegen einen anderen, `split` zerlegt den String an einem Trennzeichen in eine Liste. Die Unterabschnitte unten behandeln zuerst das Ersetzen und danach das Aufteilen.
