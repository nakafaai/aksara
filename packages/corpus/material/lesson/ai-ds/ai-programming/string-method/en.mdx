...
These methods return a new string and leave the original unchanged. That happens because a string is immutable, so every transformation produces a second value instead of editing the first. The subsections below group them by the change they make.
...
One group of methods locates text, and another group answers yes-or-no questions about it. The searching methods return a position, while the validating methods return `True` or `False`. The subsections below separate the two.
...
These two methods change how a string is divided into parts. `replace` swaps one substring for another, and `split` breaks a string into a list at a separator. The subsections below take replacement first and splitting second.
