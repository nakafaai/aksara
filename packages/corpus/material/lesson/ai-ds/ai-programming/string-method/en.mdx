...
<Highlight>Python strings are immutable</Highlight>: the value stored in a string object cannot be edited in place. An operation may create another string, and a variable may then be rebound to that new object, but the original string object remains unchanged.
...
raw_newline = r"\\n"
print(f"Raw spelling {raw_newline!r}: {len(raw_newline)} code points")`
    }
  ]}
/>
...
These methods return a new string and **leave the original unchanged**. That happens because a string is immutable, so every transformation produces a second value instead of editing the first.
...
One group of methods locates text, and another group answers yes-or-no questions about it. The searching methods **return a position**, while the validating methods return `True` or `False`.
...
The prefix `https://` classifies the text by URL scheme. It does not establish that the site is trustworthy or the address is valid.

## Replace and Split Methods

These two methods change how a string is divided into parts. `replace` **swaps one substring for another**, and `split` breaks a string into a list at a separator.
...
Transformation methods that **return strings** can be chained. A method such as `split()` returns a list, so the next operation must suit that result type. Keep a chain short enough that each step remains obvious. Use an intermediate variable when one step needs validation or explanation before the next transformation.

After the first `.strip()`, spaces remain between `+++` and `Python`. Therefore `.strip("+- ")` removes **plus signs, minus signs, and spaces** from both ends before case conversion.
...
The phone function formats only two sample patterns. Indonesian phone numbers are not limited to those lengths. A filename is assumed to be a simple name without directories. The email normalization above demonstrates string processing rather than full email-address validation.
...
def format_phone_number(phone):
    ...
    # Two sample patterns, not all Indonesian numbering rules
