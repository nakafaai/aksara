...
These methods return dynamic view objects rather than lists. A view reflects later changes to its dictionary and supports iteration, but it does not support positional indexing or item assignment. Call `list()` only when you specifically need a separate list snapshot.
