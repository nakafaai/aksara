...
A newly opened file starts at its initial stream position. Reading advances that position. In text mode, `tell()` can return an opaque position cookie rather than a simple character index, so store and reuse positions instead of calculating byte offsets yourself.
