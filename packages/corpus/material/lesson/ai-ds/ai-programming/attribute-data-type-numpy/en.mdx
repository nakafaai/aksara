...
A data type determines a value’s **representation in memory** and the operations NumPy can perform on it. A NumPy boolean occupies one byte, while a complex number stores separate real and imaginary components.

Every element in a regular NumPy array uses [the same data type](https://numpy.org/doc/stable/user/basics.types.html). This layout lets loops implemented by NumPy in C process elements without checking each value's type one by one.
...
| | `int64` | <InlineMath math="64" />-bit signed integer | <InlineMath math="-2^{63} \text{ to } 2^{63}-1" /> |
...
| | `uint64` | <InlineMath math="64" />-bit unsigned integer | <InlineMath math="0 \text{ to } 2^{64}-1" /> |
...
Attributes describe the **array structure** without reading every value: its shape, element count, dtype, and bytes occupied by elements. `nbytes` measures element storage, not all Python process memory or the array object’s metadata.
...
An array reports its own structure through attributes. The table lists the ones you read most often and the **value each one returns** for a <InlineMath math="2 \times 3" /> array.
...
All elements in a regular NumPy array share one data type. The `numpy.dtype` object explains how those values are stored and interpreted in memory. When inputs have different types, <Highlight>NumPy applies its promotion rules</Highlight> to find a compatible common dtype. Precision and type category determine the result. The number of times each input type appears does not affect this rule.
...
Convert when the **required type differs** from the current dtype, for example to satisfy a function’s input contract or reduce memory use. Conversion can lose precision or fractional values. Storing integers also does not guarantee that every later operation returns integers.
...
The following function reports **shape, dimensions, dtype, and element memory** together. The three arrays differ in their dimensions and element counts, while each uses `int64`.
...
def validate_feature_matrix(arr):
    ...
    print(f"Memory usage: {memory_mib:.2f} MiB")
