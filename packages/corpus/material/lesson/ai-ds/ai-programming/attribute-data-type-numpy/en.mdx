...
Every element stored in a NumPy array <Highlight>has a type that determines its memory representation and the operations NumPy can perform on it</Highlight>. A NumPy boolean occupies one byte, while a complex number stores separate real and imaginary components.
...
The data type an array starts with is rarely the one a calculation should end with. Converting shrinks memory use, satisfies a function that expects a particular type, and keeps integer arithmetic from turning into floating point.
