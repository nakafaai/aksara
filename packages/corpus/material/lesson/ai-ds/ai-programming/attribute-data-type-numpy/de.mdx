...
Jedes NumPy-Array besitzt Attribute für Form, Elementzahl, Datentyp und Speicherbedarf. Das Auslesen kostet fast nichts, weil diese Werte zusammen mit dem Array gespeichert sind und nicht durch Durchlaufen aller Elemente berechnet werden. Die drei Abschnitte unten behandeln die Geometrie des Arrays, die einzelnen Attributnamen und den Fall, dass die Datentypen innerhalb eines Arrays nicht einheitlich sind.
...
Der Datentyp, mit dem ein Array angelegt wird, ist selten der Datentyp, mit dem eine Berechnung enden soll. Eine Konvertierung verkleinert den Speicherbedarf, erfüllt die Anforderung einer Funktion an einen bestimmten Typ oder verhindert, dass ganzzahlige Arithmetik in Gleitkomma übergeht. Die drei Abschnitte unten behandeln die Konvertierungsmethoden, den eingesparten Speicher bei einem schmaleren Typ und die Metadaten, die einen Typ genau beschreiben.
...
Array-Attribute sind beim Arbeiten mit Daten vor allem ein Kontrollwerkzeug. Mit `shape`, `ndim` und `dtype` prüfst du, ob ein Zwischenergebnis noch die erwartete Struktur hat, und entdeckst Fehler, bevor sie sich in späteren Rechenschritten fortsetzen. Die beiden Abschnitte unten zeigen diese Kontrolle an einer Datenstruktur und beim Aufspüren fehlerhafter Werte.
