...
The core `ndarray` object represents an n-dimensional, fixed-size container whose elements share one data type. This design enables vectorized operations, where one expression describes work over an entire array. The [official NumPy documentation](https://numpy.org/doc/stable/) provides tutorials and further examples.
...
Vectorization means expressing an operation for an entire array instead of writing an explicit Python loop over individual elements. NumPy still performs the work element by element internally, but the loop runs in optimized compiled code.
...
| Data type | Inferred from input | Floating point by default; configurable with `dtype` |
