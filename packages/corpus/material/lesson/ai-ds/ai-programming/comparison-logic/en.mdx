...
Operator precedence <Highlight>determines how Python groups an expression when parentheses do not state the grouping explicitly</Highlight>. This rule describes the grouping. Python may evaluate the operands in a different order from the one you see on the page.
...
Any Python object can appear where a condition is expected, including objects that are not the values `True` and `False`. Python then decides for itself which values count as false. Under that rule an empty list, the number zero, and the empty string all behave like `False` inside an `if`, while a non-empty container behaves like `True`.
...
The diagram traces a decimal value into binary floating-point storage, which is where the comparison surprise comes from.

<Mermaid
  title="Decimals Can Compare Differently Than They Look"
  description="Decimal values become binary floats, and limited precision can cause a rounding error that makes a comparison fail. math.isclose with a relative or absolute tolerance compares the values safely."
  chart={`
    flowchart TD
