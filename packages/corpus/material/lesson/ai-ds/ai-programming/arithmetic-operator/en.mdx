...
Floor division rounds toward negative infinity. For a negative quotient such as `-7 // 2`, the result is `-4` rather than `-3`. If either operand is a float, Python still applies the floor operation but returns a float.
...
The expression `6 / 2 * 4` is evaluated as `(6 / 2) * 4 = 3 * 4 = 12` rather than `6 / (2 * 4) = 6 / 8 = 0.75`.
...
The expression `2 ** 3 ** 2` is evaluated as `2 ** (3 ** 2) = 2 ** 9 = 512` rather than `(2 ** 3) ** 2 = 8 ** 2 = 64`.
