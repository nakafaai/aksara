...
Pembagian lantai selalu membulatkan ke arah negatif tak hingga. Karena itu, `-7 // 2` menghasilkan `-4` dan bukan `-3`. Jika salah satu operand berupa float, Python tetap menerapkan pembulatan lantai, tetapi mengembalikan float.
...
Ekspresi `6 / 2 * 4` dievaluasi sebagai `(6 / 2) * 4 = 3 * 4 = 12` dan bukan `6 / (2 * 4) = 6 / 8 = 0.75`.
...
Ekspresi `2 ** 3 ** 2` dievaluasi sebagai `2 ** (3 ** 2) = 2 ** 9 = 512` dan bukan `(2 ** 3) ** 2 = 8 ** 2 = 64`.
