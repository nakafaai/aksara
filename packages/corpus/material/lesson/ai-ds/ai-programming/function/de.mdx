export const metadata = {
    title: "Funktionen",
  description: "Definiere Python-Funktionen mit Parametern, Rückgabewerten, Gültigkeitsbereichen, Closures und expliziter Fehlerbehandlung.",
...
Eine Funktion <Highlight>gibt einer wiederverwendbaren Operation einen Namen</Highlight>. Parameter beschreiben ihre Eingaben, der Funktionsrumpf erledigt die Arbeit und eine `return`-Anweisung kann dem aufrufenden Code ein Ergebnis liefern.

## Funktionsstruktur und Syntax

Jede Python-Funktion folgt derselben **Grundstruktur**:
...
# Direkter Aufruf innerhalb von print
...
1. Python wertet den Namen oder Ausdruck aus und prüft, ob das Ergebnis aufrufbar ist
...
1. **Positionsargumente folgen der Parameterreihenfolge**
2. Schlüsselwortargumente können in beliebiger Reihenfolge angegeben werden
3. Gewöhnliche Positionsargumente stehen vor Schlüsselwortargumenten
4. Parameter mit Standardwerten sind optional

### Variable Anzahl von Parametern

Mit `*args` und `**kwargs` kann eine Funktion **zusätzliche Argumente sammeln**. Die Funktion muss Anzahl, Typen und Werte weiterhin nach den Anforderungen des Programms prüfen.
...
print(f"Ergebnis der print_message-Funktion: {result}")  # Ausgabe: Ergebnis der print_message-Funktion: None`
    }
  ]}
/>
...
print(f"Endgültiger Zähler: {counter}")      # Ausgabe: Endgültiger Zähler: 1`
    }
  ]}
/>

Die Namenssuche folgt der **LEGB-Reihenfolge**:
...
In Python lassen sich Funktionen **wie andere Daten behandeln**. Du kannst sie an Namen binden, in Collections speichern, als Argument übergeben und aus anderen Funktionen zurückgeben.
...
Ein Docstring ist ein String-Literal an der **ersten Stelle** einer Funktionsdefinition. Er erklärt, was die Funktion tut und wie sie verwendet wird.
...
<CodeBlock
  data={[
    ...
    """
    Berechnet die Fakultät einer nichtnegativen ganzen Zahl.
