...
## Reusable Code with Inputs and Results

A function <Highlight>names a reusable operation</Highlight>. Its parameters describe the **inputs it accepts**, its body performs the work, and a `return` statement can provide a result to the caller.
...
Parameters are the variables written in a function definition, while arguments are the **actual values you pass** when you call that function. Python offers several kinds of parameters, and the kind a definition uses tells you whether a caller must supply that value by position, by name, or not at all.
...
1. **Positional arguments follow the parameter order**
2. Keyword arguments can be given in any order
3. Ordinary positional arguments precede keyword arguments
...
Name lookup follows **LEGB order**:
...
In Python, functions are **first-class objects**, which means you can treat them like other data. You can store them in variables, pass them as arguments, or return them from other functions.
...
<CodeBlock
  data={[
    ...
    """
    Calculate factorial of a nonnegative integer.
