...
<Highlight>String formatting controls how values appear as text.</Highlight> It can align table columns, choose a numeric notation, control precision, and insert values into readable messages. Python provides the `str.format()` method and f-strings for this work.
...
Digits after the point and the way a sign is shown are controlled separately. Both settings shape how one number is rendered: the precision field limits how much of the value survives, and the sign flag decides how the value announces whether it is positive or negative.
...
An f-string keeps an expression and its surrounding text in one place, so the expression is evaluated when that line runs and the result reflects the current values. A template stored as a string does the same work when the text is not fixed while you write the code.
