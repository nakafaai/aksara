...
A field of fixed width needs a rule for where the text sits and what fills the rest. The subsections below cover alignment and the fill character.
...
Digits after the point and the way a sign is shown are controlled separately. The subsections below take precision first and the sign flag second.
...
An f-string keeps an expression and its surrounding text in one place. The subsections below cover its version requirement and the case where the template itself is built at runtime.
