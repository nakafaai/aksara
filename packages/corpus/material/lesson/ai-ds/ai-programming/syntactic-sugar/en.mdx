...
<Highlight>Syntactic sugar is a concise notation for behavior that could also be expressed more explicitly.</Highlight> It can remove repetitive ceremony, but shorter syntax is useful only when the reader can still see what the code does.
...
Python has other compact syntax for common operations. These forms can improve readability, but they do not guarantee better runtime performance. Each form replaces a longer statement without changing the result the program computes.
