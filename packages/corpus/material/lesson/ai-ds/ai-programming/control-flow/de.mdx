...
`zip()` verbindet jeweils die Werte an derselben Iterationsposition mehrerer Iterables zu einem Tupel. Die Iteration endet, sobald das kürzeste Iterable keine Elemente mehr liefert, die Zahl der Tupel entspricht also der Länge des kürzesten Eingabeobjekts. Die Abschnitte unten behandeln den einfachen Aufruf, das Umwandeln der Tupel in eine andere Sammlung und das erneute Entpacken.
