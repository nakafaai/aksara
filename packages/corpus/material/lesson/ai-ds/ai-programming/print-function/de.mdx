...
## Werte als Text ausgeben

Die eingebaute Funktion `print()` <Highlight>schreibt die Textdarstellung von Werten in einen Stream</Highlight>. Standardmäßig ist das `sys.stdout`, dessen Inhalt ein Terminal gewöhnlich auf dem Bildschirm zeigt. Die Ausgabe kann aber auch in eine Datei oder an ein anderes Ziel umgeleitet werden.
...
Mehrere **Ausgabeparameter** steuern, welche Werte `print()` ausgibt und wie diese Ausgabe formatiert wird.
...
Der Parameter `sep` bestimmt, welche Zeichenfolge mehrere Objekte in einem Aufruf voneinander trennt. <Highlight>Der Standardwert ist ein einzelnes Leerzeichen</Highlight>.
...
Der Parameter `end` legt das Zeichen oder die Zeichenfolge fest, die nach den ausgegebenen Objekten steht. Standardmäßig ist das ein **Zeilenumbruch** (`\n`), sodass der nächste Aufruf von `print` in einer neuen Zeile beginnt.
...
Ein einziger Aufruf von `print()` kann Werte verschiedener Typen annehmen, sie in Text umwandeln und der Reihe nach ausgeben. Das genügt für eine schnelle Prüfung während der Programmausführung.

## Ausgabeformatierungstechniken

Ein einfacher `print()`-Aufruf reicht für einen schnellen Blick, und eine saubere Ausgabe braucht ein weiteres Werkzeug. Ein f-String baut **einen Textwert** aus Ausdrücken, die direkt neben ihren umgebenden Wörtern stehen, und das Argument `end` bestimmt, was `print()` nach diesem Wert ausgibt. Beides hält die Ausgabe lesbar, ohne dass du die Teile vorher getrennt zusammenfügen musst.

### Ausgabe mit formatierten Strings
...
`end` hält jede Aktualisierung in der aktuellen Zeile. `flush=True` fordert Python auf, gepufferte Ausgabe sofort zu schreiben. Eine echte Fortschrittsanzeige muss außerdem die Fähigkeiten des Terminals berücksichtigen, denn Tabulatoren und Zeichenbreiten können je nach Terminal anders aussehen.
