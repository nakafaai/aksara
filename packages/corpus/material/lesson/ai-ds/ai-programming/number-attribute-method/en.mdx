...
For example, <InlineMath math="5" /> creates an `int` and <InlineMath math="3.14" /> creates a `float`. Numeric objects are immutable, so their methods return information or new values rather than changing the original number.
