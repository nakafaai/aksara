...
A list has three properties that decide how you work with it. The table names each one and gives a short example.
...
Python gives you three more built-in containers, and each one keeps a different promise about order, duplicates, and whether you may change it after creation. A tuple fixes its positions, a dictionary maps each unique key to one value, and a set keeps <Highlight>only unique items</Highlight> without promising an order.
