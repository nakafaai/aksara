...
Operator `in` dan `not in` memeriksa apakah suatu objek memuat nilai tertentu. Cara pemeriksaannya bergantung pada tipe objek tersebut: list menelusuri elemennya, dictionary mencari pada daftar kunci, dan string mencari potongan teks. Bagian di bawah ini menunjukkan perbedaan itu dalam pemakaian nyata.
