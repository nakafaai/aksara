...
The operators `in` and `not in` ask whether a container contains a value. Each container type defines how membership is checked: a list scans its elements, a dictionary looks up its keys, and a string looks for a substring. The sections below show those differences in practice.
