...
An iterable does not have to be ordered, indexable, reusable, or finite. Calling `iter()` must return an iterator that yields items. A finite iterator stops when it is exhausted.

### Types of Iterables

Strings, lists, and tuples yield **one item per iteration**. The examples below show a character, a number, or a coordinate being passed to the loop variable.
...
A list can be traversed again, but a generator object **remembers its iteration progress**. The first `list(iterator)` exhausts the generator below. The second call does not restart the computation, so it returns an empty list.
...
The operators `in` and `not in` ask whether a container **contains a value**. Each container type defines how membership is checked: a list scans its elements, a dictionary looks up its keys, and a string looks for a substring.

### Using in and not in Operators

Lists and tuples compare elements until a match or the end. Strings search for a substring. Sets and dictionaries use their own lookup behavior. In a dictionary, `in` checks **keys, not values**.
...
The built-ins `any()` and `all()` consume an iterable and test the truthiness of its items. Both <Highlight>stop as soon as the result is known</Highlight>, so generator expressions avoid building an unnecessary intermediate list.
