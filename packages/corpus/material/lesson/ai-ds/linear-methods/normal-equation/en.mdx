...
Explicitly forming <InlineMath math="A^{\mathsf T}A" /> can therefore magnify sensitivity and rounding effects. The inverse formula proves an algebraic identity. In software, use QR for a well-determined full-rank least-squares solve and SVD when rank deficiency or severe ill-conditioning matters.
