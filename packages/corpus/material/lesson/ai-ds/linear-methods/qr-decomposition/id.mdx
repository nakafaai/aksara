...
Kita ingin mengubah matriks menjadi bentuk segitiga atas melalui transformasi ortogonal yang lebih stabil secara numerik daripada operasi baris elementer. Bayangkan seperti memutar dan memantulkan ruang geometris untuk menyederhanakan matriks tanpa mengubah sifat dasarnya.
...
Gram-Schmidt klasik memberikan penurunan yang jelas, tetapi dapat kehilangan ortogonalitas dalam aritmetika titik mengambang ketika kolom-kolomnya hampir bergantung linear. Gram-Schmidt termodifikasi memperbaiki perilaku tersebut. QR Householder merupakan implementasi padat yang lazim dan stabil secara numerik karena menerapkan reflektor ortogonal secara langsung.
...
Hasil perkalian ini adalah matriks <InlineMath math="m \times m" /> dengan peringkat <InlineMath math="1" /> untuk <InlineMath math="v \neq 0" />.
...
3. Perkalian <InlineMath math="S \cdot x" /> dari <InlineMath math="S" /> dari kiri dengan vektor <InlineMath math="x \in \mathbb{R}^m" />
...
Untuk <InlineMath math="m\ge n" />, QR Householder memerlukan kira-kira
...
2. Elemen diagonal <InlineMath math="R" /> adalah bilangan <InlineMath math="\pm\|\tilde{a}_i\|_2" />
   ...
   dan pernyataan keunikan untuk peringkat penuh tidak lagi berlaku. QR tetap ada, tetapi QR penyingkap-rank dengan pivot kolom lebih tepat ketika peringkat numerik penting.
...
3. Sebagai pengganti membangun <InlineMath math="Q" />, dalam penyimpanan kompak juga dapat menyimpan
   ...
   <BlockMath math="\text{Simpan vektor Householder }u_i" />
