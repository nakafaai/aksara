...
Why is it called economical? It saves storage space and computational time.
We keep <InlineMath math="Q_1" /> of size <InlineMath math="m \times n" />. The full matrix <InlineMath math="Q" />
would have size <InlineMath math="m \times m" /> and can be very large.
...
Here, the kernel of <InlineMath math="A^T" /> is the set of all vectors <InlineMath math="x" /> that satisfy <InlineMath math="A^T x = 0" />.
...
Classical Gram-Schmidt gives a clear derivation but can lose orthogonality in floating-point arithmetic when the columns are nearly dependent. Modified Gram-Schmidt improves that behavior. Householder QR is the usual dense, numerically stable implementation because it applies orthogonal reflectors directly.
...
3. Multiplication <InlineMath math="S \cdot x" /> of <InlineMath math="S" /> from the left with vector <InlineMath math="x \in \mathbb{R}^m" />
...
For <InlineMath math="m\ge n" />, Householder QR requires approximately
...
2. The diagonal elements of <InlineMath math="R" /> are the numbers <InlineMath math="\pm\|\tilde{a}_i\|_2" />
   ...
   and the full-rank uniqueness statement no longer applies. QR still exists, but rank-revealing QR with column pivoting is preferable when numerical rank matters.
...
3. Compact storage keeps only the information needed for the transformation,
   so constructing <InlineMath math="Q" /> is unnecessary:

   <BlockMath math="\text{Store the Householder vector }u_i" />
