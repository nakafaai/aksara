...
Now, look at a very important relationship: <InlineMath math="\lambda \in \mathbb{K}" /> is an eigenvalue of <InlineMath math="A" /> if and only if <InlineMath math="\det(A - \lambda \cdot I) = 0" />.
...
Thus similarity preserves eigenvalues while the inverse basis change converts the corresponding eigenvectors.
