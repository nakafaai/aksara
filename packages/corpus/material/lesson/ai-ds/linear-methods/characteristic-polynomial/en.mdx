...
This degree-<InlineMath math="n" /> polynomial in <InlineMath math="t" />, with coefficients <InlineMath math="a_0,\ldots,a_n\in\mathbb{K}" />, is the **characteristic polynomial** of <InlineMath math="A" />. This lesson uses the convention <InlineMath math="\det(A-tI)" />. The equally common convention <InlineMath math="\det(tI-A)" /> differs by the factor <InlineMath math="(-1)^n" /> but has the same roots.
