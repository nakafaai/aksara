...
Two vectors are equal, or represented by equivalent directed segments, when their components are equal. For nonzero vectors, this is the same as having equal magnitude and the same direction. For vectors <InlineMath math="\vec{a}" /> and <InlineMath math="\vec{b}" />, we write <InlineMath math="\vec{a} = \vec{b}" />.
