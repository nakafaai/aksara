...
Scalar multiplication combines a real number, called a scalar, with a vector <InlineMath math="\vec{v}" />. The result is a new vector whose magnitude is scaled by the scalar's absolute value. For a nonzero original vector, the sign determines whether the direction stays the same or reverses.
...
Multiplying a vector by a number changes its length by that factor and may also reverse it. The two subsections below describe the change geometrically and in components.
...
A factor greater than one stretches the vector, a factor between zero and one shrinks it, and a negative factor also reverses the direction. The examples below compare one positive and one negative multiplier.

<Highlight>The sign of the multiplier decides</Highlight> whether the result keeps the original direction. The two examples below work one positive and one negative multiplier.
...
It is proven that the three vectors <InlineMath math="\overrightarrow{BR}" />, <InlineMath math="\overrightarrow{BU}" />, and <InlineMath math="\overrightarrow{UR}" /> have the same direction.
