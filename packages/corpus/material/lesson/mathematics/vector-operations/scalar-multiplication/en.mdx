...
<Highlight>The result is a new vector whose magnitude is scaled by the scalar's absolute value.</Highlight> For a nonzero original vector, the sign determines whether the direction stays the same or reverses.
