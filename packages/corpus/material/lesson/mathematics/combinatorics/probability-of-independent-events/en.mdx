...
Events <InlineMath math="A" /> and <InlineMath math="B" /> are **independent** when conditioning on either event leaves the probability of the other unchanged. The probability model determines this property. Timing alone does not.
...
- **Verify independence** from the model or conditional probabilities rather than from timing alone
