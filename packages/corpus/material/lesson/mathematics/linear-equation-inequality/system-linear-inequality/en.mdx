...
o or more inequalities with two variables (usually <InlineMath math="x" /> and <InlineMath math="y" />):
...
A linear inequality system describes the set of combinations that are allowed together. A word problem has to be turned into such a system before it can be solved, and the result is a region rather than a single point.
...
The two system types differ in what counts as an answer. The table sets the operation sign, the solution form, and the graphical picture side by side.
...
The two boundary lines meet at <InlineMath math="(5,5)" />. This point forms one corner of the feasible set. The shaded region contains the full set of solutions.
