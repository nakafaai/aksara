...
A linear equation system is solved by removing one variable at a time until a single value remains. The two subsections below use substitution and elimination on the same kind of system, so you can compare when each one is the shorter route.
...
Applied systems start from a word problem. This section shows how to build the model, solve it, and then check that the solution answers the original question.
...
- <InlineMath math="a" /> = number of <InlineMath math="1" />-point shots
- <InlineMath math="b" /> = number of <InlineMath math="2" />-point shots
- <InlineMath math="c" /> = number of <InlineMath math="3" />-point shots
