export const metadata = {
  ...
  description: "Learn inverse formulas for order-two and order-three matrices through determinants, adjugates, and linear equation examples.",
...
Verify the result rather than trusting the formula alone:
...
where <InlineMath math="A" /> is the coefficient matrix, <InlineMath math="X" /> is the variable matrix, and <InlineMath math="B" /> is the constant matrix. If matrix <InlineMath math="A" /> has an inverse (<InlineMath math="A^{-1}" />), multiply both sides from the left. The side matters because matrix multiplication is not generally commutative:
...
4.  **Comparison of <InlineMath math="X^{-1} + Y^{-1}" /> and <InlineMath math="(X+Y)^{-1}" />:**
    ...
    Inversion is not distributive over addition. The same warning already holds for ordinary reciprocals: in general, <InlineMath math="1/x+1/y\neq1/(x+y)" />.
