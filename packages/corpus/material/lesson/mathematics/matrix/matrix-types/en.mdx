export const metadata = {
  title: "Matrix Types",
  description: "Classify matrices as row, column, square, triangular, diagonal, identity, zero, or symmetric matrices.",
...
A square matrix is a matrix that <Highlight>has the same number of rows and columns</Highlight>.
...
A wide matrix is a rectangular matrix with **more columns than rows** (<InlineMath math="n > m" />).

Example:

<BlockMath math="D = \begin{bmatrix} 1 & 0 & 4 \\ -2 & 3 & 5 \end{bmatrix}" />

Matrix <InlineMath math="D" /> is a wide matrix of order <InlineMath math="2 \times 3" />.
### Tall Matrix

A tall matrix is a rectangular matrix with **more rows than columns** (<InlineMath math="m > n" />).

Example:

<BlockMath math="T = \begin{bmatrix} 7 & 1 \\ 0 & -3 \\ 4 & 2 \end{bmatrix}" />

Matrix <InlineMath math="T" /> is a tall matrix of order <InlineMath math="3 \times 2" />.
## Triangular Matrix
...
In symbols, <InlineMath math="a_{ij} = 0" /> whenever <InlineMath math="i > j" />.
...
In symbols, <InlineMath math="a_{ij} = 0" /> whenever <InlineMath math="i \neq j" />. Elements on the main diagonal can be zero or non-zero.
...
The identity matrix is the neutral element of matrix multiplication. If <InlineMath math="A" /> has dimensions <InlineMath math="m \times n" />, the identity matrix must match the side on which it is multiplied:
...
For every pair of indices <InlineMath math="i" /> and <InlineMath math="j" />, a symmetric matrix satisfies <InlineMath math="a_{ij} = a_{ji}" />. Entries on opposite sides of the main diagonal are equal.
...
## One Matrix Can Belong to Several Types

These names **are not mutually exclusive**. An identity matrix is both diagonal and symmetric. However, a nonsquare zero matrix is neither diagonal nor symmetric under the definitions above. Check the shape first, then the pattern of entries.

| Example | Applicable types | Reason |
| --- | --- | --- |
| <InlineMath math="I_3" /> | Identity, diagonal, upper and lower triangular, symmetric | Every off-diagonal entry is zero and every main-diagonal entry is one. |
| <InlineMath math="O_{2\times3}" /> | Zero, rectangular, wide | Every entry is zero, but the row and column counts differ. |
