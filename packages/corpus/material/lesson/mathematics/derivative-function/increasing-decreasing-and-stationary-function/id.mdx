...
Secara geometris, <InlineMath math="f'(x)" /> adalah gradien [garis singgung](/id/materi/matematika/turunan-fungsi/persamaan-garis-singgung-pada-kurva). Tandanya menunjukkan arah lokal kurva.
...
Titik stasioner membagi domain menjadi beberapa interval yang dapat diuji. Titik ini merupakan calon tempat perubahan kemonotonan, tetapi fungsi hanya beralih dari naik ke turun atau sebaliknya <Highlight>jika tanda turunannya berubah</Highlight>.

Saat menganalisis fungsi lain, periksa juga **batas domain dan titik yang tidak dapat diturunkan**. Misalnya, domain <InlineMath math="1/x" /> terpisah di <InlineMath math="x=0" /> meskipun fungsi itu tidak mempunyai titik stasioner. Uji tanda harus dilakukan pada setiap interval domain yang terpisah.
...
| Interval | Titik uji | Nilai turunan | Perilaku |
| --- | --- | --- | --- |
| <InlineMath math="x<-1" /> | <InlineMath math="x=-2" /> | <InlineMath math="f'(-2)=3(-2)^2-3=9" /> | Naik |
| <InlineMath math="-1<x<1" /> | <InlineMath math="x=0" /> | <InlineMath math="f'(0)=3(0)^2-3=-3" /> | Turun |
| <InlineMath math="x>1" /> | <InlineMath math="x=2" /> | <InlineMath math="f'(2)=3(2)^2-3=9" /> | Naik |
...
**Soal 1**. Turunan pertama dari <InlineMath math="f(x) = x^3 - 3x^2 - 15" /> adalah <InlineMath math="f'(x) = 3x^2 - 6x" />.
...
**Soal 2**. Agar fungsi ini naik di seluruh interval, cukup mensyaratkan <InlineMath math="f'(x) \ge 0" /> pada setiap titik dalam interval tersebut.
...
Pada batas <InlineMath math="a=-1/3" />, turunan menjadi <InlineMath math="f'(x)=x(2-x)>0" /> untuk setiap <InlineMath math="0<x<2" />. Jadi, fungsi tetap **naik di seluruh interval terbuka**. Jika <InlineMath math="a<-1/3" />, turunan negatif untuk nilai <InlineMath math="x" /> yang cukup dekat dengan <InlineMath math="2" />, sehingga fungsi tidak terus naik.

**Soal 3**. Gunakan identitas trigonometri sudut ganda: <InlineMath math="\sin(2A) = 2\sin(A)\cos(A)" />.
