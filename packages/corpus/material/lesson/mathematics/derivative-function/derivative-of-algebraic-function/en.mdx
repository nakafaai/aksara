...
> Both methods give the same result. Simplifying first is shorter here, while the product-rule method confirms why the result is valid. As a real-valued function, the original expression has domain <InlineMath math="x \ge 0" />; the differentiation above is carried out on its interior, <InlineMath math="x>0" />.
