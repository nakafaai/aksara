...
In a **composite function**, the output of one function becomes the input of another. If <InlineMath math="g(x)" /> is the **inner function** and <InlineMath math="f(u)" /> is the **outer function**, their composition is <InlineMath math="y = f(g(x))" />.
...
For a composite function, we differentiate each layer in order and multiply the resulting derivatives.
