...
**Problem 1**. For <InlineMath math="y=(3x^2-1)^3" />, cubing is the **outer function** and the quadratic expression is the **inner function**. Keep the quadratic expression in place while differentiating the cube.
...
**Problem 2**. For <InlineMath math="y=\sin^3(2x)" />, **three layers** produce three derivative factors. Write the linear function, sine, and cube separately so that the linear function’s factor is retained.
