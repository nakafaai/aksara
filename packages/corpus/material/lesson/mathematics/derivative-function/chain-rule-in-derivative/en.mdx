export const metadata = {
  ...
  description: "Differentiate composite functions with the chain rule in power and trigonometric examples.",
...
In a **[composite function](/en/subjects/mathematics/function-composition-inverse-function/function-composition)**, the output of one function becomes the input of another. If <InlineMath math="g(x)" /> is the **inner function** and <InlineMath math="f(u)" /> is the **outer function**, their composition is <InlineMath math="y = f(g(x))" />.
...
> If <InlineMath math="y = f(g(x))" />, with <InlineMath math="g" /> differentiable at <InlineMath math="x" /> and <InlineMath math="f" /> differentiable at <InlineMath math="g(x)" />, then the derivative is the derivative of the outer function evaluated at the inner function, multiplied by the derivative of the inner function.
...
This is the **chain rule**. <Highlight>Work from the outermost layer inward</Highlight>, keep each inner expression in place until its turn, and multiply the derivatives.
...
Angles in the trigonometric functions are measured in **radians**, so the sine and cosine derivative rules apply without an angle-conversion factor.
...
### Differentiating a Power of a Quadratic

For <InlineMath math="y=(3x^2-1)^3" />, cubing is the outer function and the quadratic expression is the inner function. Keep the quadratic expression in place while differentiating the cube.
...
### Differentiating a Power of Sine

For <InlineMath math="y=\sin^3(2x)" />, three layers produce three derivative factors. Write the linear function, sine, and cube separately so that the linear function’s factor is retained.
