...
The roots are the values of <InlineMath math="x" /> that make the equation equal to zero. <Highlight>In product form, we can find them directly by setting each factor equal to zero.</Highlight>
...
The three examples below climb the same four steps: first with <InlineMath math="a = 1" />, then with a negative constant term, then with <InlineMath math="a \neq 1" />. Watch for the factor pair with product <InlineMath math="ac" /> and sum <InlineMath math="b" /> in each one.
