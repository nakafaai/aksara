...
Remember that a perfect square trinomial follows the pattern <InlineMath math="x^2 + 2px + p^2 = (x + p)^2" />. We use this pattern to transform quadratic equations into a more solvable form. When the middle term is <InlineMath math="2px" />, the term needed to complete the square is <InlineMath math="p^2" />.
