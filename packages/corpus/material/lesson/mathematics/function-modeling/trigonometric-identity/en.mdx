
export const metadata = {
  title: "Trigonometric Identity",
    description: "Prove Pythagorean, reciprocal, and quotient identities, then use them to simplify trigonometric expressions.",
  authors: [{ name: "Nabil Akbarazzima Fatih" }],
  datePublished: "2025-05-18",
  dateModified: "2026-09-23",
  subject: "Functions and Their Modeling",
};

## Equations That Hold for Every Permitted Angle

Some equations are true for every permitted value of their variables. For example, <InlineMath math="(a + b)^2 = a^2 + 2ab + b^2" /> holds for all values of <InlineMath math="a" /> and <InlineMath math="b" />. Such an equation is called an identity.

In trigonometry, an equation that is <Highlight>true for every angle in its domain</Highlight> is a **trigonometric identity**. An identity rewrites an expression into an equivalent form without changing its value. This is useful when proving another identity or solving an equation.

## Basic Trigonometric Identities

An identity connects **two equal-valued forms** on their common domain. You may exchange the forms provided every denominator remains defined. Rewriting must not silently extend the domain.

### Pythagorean Identity

Consider a unit circle with point <InlineMath math="P(x, y)" /> that forms angle <InlineMath math="\theta" /> with the positive <InlineMath math="x" />-axis.

<UnitCircle
  title="Unit Circle and Pythagorean Identity"
  description={
    <>
      Notice how the coordinates of point <InlineMath math="P" /> change as the
      angle changes. These coordinates are the values of{" "}
      <InlineMath math="\cos\theta" /> and <InlineMath math="\sin\theta" />.
    </>
  }
  angle={45}
/>

On the unit circle:

- Radius: <InlineMath math="r = 1" />
- The <InlineMath math="x" />-coordinate is <InlineMath math="\cos \theta" />
- The <InlineMath math="y" />-coordinate is <InlineMath math="\sin \theta" />

Using the Pythagorean theorem for point <InlineMath math="P" />:

<BlockMath math="x^2 + y^2 = 1^2" />

Substituting the values of <InlineMath math="x" /> and <InlineMath math="y" />:

<BlockMath math="(\cos \theta)^2 + (\sin \theta)^2 = 1" />

Or can be written as:

<BlockMath math="\sin^2 \theta + \cos^2 \theta = 1" />

This equation is the **Pythagorean identity**.

**Other Forms of Pythagorean Identity**:

From the basic identity above, we can derive two other forms:

**Second form**: Divide both sides by <InlineMath math="\cos^2 \theta" /> (for <InlineMath math="\cos \theta \neq 0" />)

<MathContainer>

<BlockMath math="\frac{\sin^2 \theta}{\cos^2 \theta} + \frac{\cos^2 \theta}{\cos^2 \theta} = \frac{1}{\cos^2 \theta}" />

<BlockMath math="\tan^2 \theta + 1 = \sec^2 \theta" />
</MathContainer>

**Third form**: Divide both sides by <InlineMath math="\sin^2 \theta" /> (for <InlineMath math="\sin \theta \neq 0" />)

<MathContainer>

<BlockMath math="\frac{\sin^2 \theta}{\sin^2 \theta} + \frac{\cos^2 \theta}{\sin^2 \theta} = \frac{1}{\sin^2 \theta}" />

<BlockMath math="1 + \cot^2 \theta = \csc^2 \theta" />
</MathContainer>

### Reciprocal Identities

Each trigonometric function has its reciprocal. This relationship forms reciprocal identities:

<MathContainer>

<BlockMath math="\sin \theta = \frac{1}{\csc \theta}" />

<BlockMath math="\cos \theta = \frac{1}{\sec \theta}" />

<BlockMath math="\tan \theta = \frac{1}{\cot \theta}" />
</MathContainer>

Equivalently:

<MathContainer>

<BlockMath math="\csc \theta = \frac{1}{\sin \theta}" />

<BlockMath math="\sec \theta = \frac{1}{\cos \theta}" />

<BlockMath math="\cot \theta = \frac{1}{\tan \theta}" />
</MathContainer>

Each reciprocal identity applies wherever both sides are defined.

Check the **nonzero denominators** in each pair. A multiplicative reciprocal differs from an inverse function such as arcsine.

| Relationship | Condition on both sides |
| --- | --- |
| <InlineMath math="\sin\theta=1/\csc\theta" /> | <InlineMath math="\sin\theta\neq0" /> |
| <InlineMath math="\cos\theta=1/\sec\theta" /> | <InlineMath math="\cos\theta\neq0" /> |
| <InlineMath math="\tan\theta=1/\cot\theta" /> | <InlineMath math="\sin\theta\neq0,\ \cos\theta\neq0" /> |

### Quotient Identities

Quotient identities relate tangent and cotangent to sine and cosine:

<ContentStack>

  <MathContainer>

<BlockMath math="\tan \theta = \frac{\sin \theta}{\cos \theta}" />

<BlockMath math="\cot \theta = \frac{\cos \theta}{\sin \theta}" />
</MathContainer>

The quotient forms likewise require a nonzero denominator.

  <Triangle
    title="Trigonometric Function Visualization"
    description={<>Observe how the ratios of triangle sides give <InlineMath math="\sin" />, <InlineMath math="\cos" />, and <InlineMath math="\tan" /> values. Also notice how these values change as the angle changes.</>}
    angle={30}
    labels={{
      opposite: "Opposite Side",
      adjacent: "Adjacent Side",
      hypotenuse: "Hypotenuse",
    }}
  />
</ContentStack>

Both quotient identities follow directly from the definitions of the trigonometric functions on the **unit circle**.

### Even and Odd Function Identities

When angles are negative, trigonometric functions have special properties:

Even function (symmetry about <InlineMath math="y" />-axis):

<BlockMath math="\cos(-\theta) = \cos \theta" />

**Odd functions (symmetry about origin)**:

<ContentStack>

  <MathContainer>

<BlockMath math="\sin(-\theta) = -\sin \theta" />

<BlockMath math="\tan(-\theta) = -\tan \theta" />
</MathContainer>

  <UnitCircle
    title="Exploring Even and Odd Properties"
    description={<>Enter negative and positive angles. Notice how <InlineMath math="\cos" />, <InlineMath math="\sin" />, and <InlineMath math="\tan" /> values change for opposite angles.</>}
    angle={60}
  />
</ContentStack>

## Using Identities in Proofs

Start from **one side** and rewrite it using known identities until it matches the other side. Each step preserves the value and the domain conditions.

### Simplifying Expressions

Simplify <InlineMath math="\frac{\sin^2 \theta}{\cos \theta} + \cos \theta" />

**Solution**:

<MathContainer>

<BlockMath math="\frac{\sin^2 \theta}{\cos \theta} + \cos \theta = \frac{\sin^2 \theta}{\cos \theta} + \frac{\cos^2 \theta}{\cos \theta}" />

<BlockMath math="\frac{\sin^2 \theta}{\cos \theta} + \cos \theta = \frac{\sin^2 \theta + \cos^2 \theta}{\cos \theta}" />
</MathContainer>
  Use the Pythagorean identity:

<MathContainer>

<BlockMath math="\frac{\sin^2 \theta}{\cos \theta} + \cos \theta = \frac{1}{\cos \theta}" />

<BlockMath math="\frac{\sin^2 \theta}{\cos \theta} + \cos \theta = \sec \theta" />
</MathContainer>

### Proving Identities

Prove that <InlineMath math="\frac{1 + \tan^2 \theta}{\sec \theta} = \sec \theta" />

**Solution**:

We start from the left side:

Use <InlineMath math="1+\tan^2\theta=\sec^2\theta" />:

<MathContainer>

<BlockMath math="\frac{1 + \tan^2 \theta}{\sec \theta} = \frac{\sec^2 \theta}{\sec \theta}" />

<BlockMath math="\frac{1 + \tan^2 \theta}{\sec \theta} = \sec \theta" />
</MathContainer>

The left-hand side has become the right-hand side, so the identity holds on their common domain.

## Determining Trigonometric Function Values

The **quadrant determines the sign** when an identity gives two possible square roots. For example, cosine squared determines the magnitude of cosine, while the quadrant selects the positive or negative root.

### Identity Applications

If <InlineMath math="\sin \theta = \frac{3}{5}" /> and <InlineMath math="90^\circ < \theta < 180^\circ" /> (quadrant II), determine the values of other trigonometric functions.

**Solution**:

Use the Pythagorean identity to find <InlineMath math="\cos \theta" />:

<MathContainer>

<BlockMath math="\sin^2 \theta + \cos^2 \theta = 1" />

<BlockMath math="\left(\frac{3}{5}\right)^2 + \cos^2 \theta = 1" />

<BlockMath math="\frac{9}{25} + \cos^2 \theta = 1" />

<BlockMath math="\cos^2 \theta = 1 - \frac{9}{25} = \frac{16}{25}" />

<BlockMath math="\cos \theta = \pm \frac{4}{5}" />
</MathContainer>

Since <InlineMath math="\theta" /> is in quadrant II, then <InlineMath math="\cos \theta < 0" />.
<InlineMath math="\cos \theta = -\frac{4}{5}" />

Next, calculate the other trigonometric functions:

<MathContainer>

<BlockMath math="\tan \theta = \frac{\sin \theta}{\cos \theta} = \frac{3/5}{-4/5} = -\frac{3}{4}" />

<BlockMath math="\csc \theta = \frac{1}{\sin \theta} = \frac{5}{3}" />

<BlockMath math="\sec \theta = \frac{1}{\cos \theta} = -\frac{5}{4}" />

<BlockMath math="\cot \theta = \frac{1}{\tan \theta} = -\frac{4}{3}" />
</MathContainer>

## Exercises

Each problem asks you to simplify an expression or prove an identity. Convert every function to **sine and cosine** first, then simplify.

1. Simplify the expression <InlineMath math="\frac{\tan \theta \cdot \cos \theta}{\sin \theta}" />

2. Prove the identity <InlineMath math="\frac{\sin \theta}{1 + \cos \theta} = \frac{1 - \cos \theta}{\sin \theta}" />

3. If <InlineMath math="\cos \theta = \frac{5}{13}" /> and <InlineMath math="270^\circ < \theta < 360^\circ" />, determine the values of all trigonometric functions.

4. Simplify <InlineMath math="\sin^4 \theta - \cos^4 \theta" />

5. If <InlineMath math="\tan \theta = \frac{4}{3}" /> and <InlineMath math="\sin \theta < 0" />, determine the values of <InlineMath math="\sin \theta" /> and <InlineMath math="\cos \theta" />.

### Worked Solutions

**Problem 1**. Simplify the expression step by step:

<MathContainer>

<BlockMath math="\frac{\tan \theta \cdot \cos \theta}{\sin \theta} = \frac{\frac{\sin \theta}{\cos \theta} \cdot \cos \theta}{\sin \theta}" />

<BlockMath math="\frac{\tan \theta \cdot \cos \theta}{\sin \theta} = \frac{\sin \theta}{\sin \theta} = 1" />
</MathContainer>

The result is valid on the **original expression's domain**, where <InlineMath math="\sin\theta \neq 0" /> and <InlineMath math="\cos\theta \neq 0" />.

**Problem 2**. To prove the identity, we will transform the **left side**:

<MathContainer>

<BlockMath math="\frac{\sin \theta}{1 + \cos \theta} = \frac{\sin \theta}{1 + \cos \theta} \cdot \frac{1 - \cos \theta}{1 - \cos \theta}" />

<BlockMath math="\frac{\sin \theta}{1 + \cos \theta} = \frac{\sin \theta (1 - \cos \theta)}{(1 + \cos \theta)(1 - \cos \theta)}" />

<BlockMath math="\frac{\sin \theta}{1 + \cos \theta} = \frac{\sin \theta (1 - \cos \theta)}{1 - \cos^2 \theta}" />

<BlockMath math="\frac{\sin \theta}{1 + \cos \theta} = \frac{\sin \theta (1 - \cos \theta)}{\sin^2 \theta}" />

<BlockMath math="\frac{\sin \theta}{1 + \cos \theta} = \frac{1 - \cos \theta}{\sin \theta}" />
</MathContainer>

This proves equality wherever both original sides are defined.

**Problem 3**. Given <InlineMath math="\cos \theta = \frac{5}{13}" /> in **quadrant** IV.

Finding <InlineMath math="\sin \theta" />:

<BlockMath math="\begin{aligned} \sin^2 \theta &amp;= 1 - \cos^2 \theta \\ &amp;= 1 - \frac{25}{169} \\ &amp;= \frac{144}{169} \end{aligned}" />
The value is negative in quadrant IV:

<BlockMath math="\sin\theta=-\frac{12}{13}" />

Other trigonometric functions:

<MathContainer>

<BlockMath math="\tan \theta = \frac{-12/13}{5/13} = -\frac{12}{5}" />

<BlockMath math="\csc \theta = -\frac{13}{12}" />

<BlockMath math="\sec \theta = \frac{13}{5}" />

<BlockMath math="\cot \theta = -\frac{5}{12}" />
</MathContainer>

**Problem 4**. Use **difference of squares** factoring:

<MathContainer>

<BlockMath math="\begin{aligned} &amp;\sin^4\theta-\cos^4\theta \\ &amp;=(\sin^2\theta)^2-(\cos^2\theta)^2 \end{aligned}" />

<BlockMath math="\sin^4\theta-\cos^4\theta = (\sin^2 \theta + \cos^2 \theta)(\sin^2 \theta - \cos^2 \theta)" />

<BlockMath math="\sin^4\theta-\cos^4\theta = 1 \cdot (\sin^2 \theta - \cos^2 \theta)" />

<BlockMath math="\sin^4\theta-\cos^4\theta = \sin^2 \theta - \cos^2 \theta" />
</MathContainer>

**Problem 5**. Given <InlineMath math="\tan \theta = \frac{4}{3}" /> and <InlineMath math="\sin \theta < 0" />.

Since <InlineMath math="\tan \theta > 0" /> and <InlineMath math="\sin \theta < 0" />, then <InlineMath math="\cos \theta < 0" /> (**quadrant** III).

Use the identity <InlineMath math="1 + \tan^2 \theta = \sec^2 \theta" />:

<MathContainer>

<BlockMath math="1 + \frac{16}{9} = \sec^2 \theta" />

<BlockMath math="\sec^2 \theta = \frac{25}{9}" />
</MathContainer>
The value is negative in quadrant III:

<MathContainer>

<BlockMath math="\sec\theta=-\frac53" />

<BlockMath math="\cos \theta = -\frac{3}{5}" />
</MathContainer>

For <InlineMath math="\sin \theta" />:

<MathContainer>

<BlockMath math="\tan \theta = \frac{\sin \theta}{\cos \theta}" />

<BlockMath math="\frac{4}{3} = \frac{\sin \theta}{-3/5}" />

<BlockMath math="\sin \theta = \frac{4}{3} \cdot \left(-\frac{3}{5}\right) = -\frac{4}{5}" />
</MathContainer>
