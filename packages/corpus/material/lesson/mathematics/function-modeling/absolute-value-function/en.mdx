...
Absolute value functions can be transformed in various ways. A vertical stretch or a vertical shift moves the whole V, while a reflection or a horizontal shift changes its orientation. The subsections below apply each change to the parent function <InlineMath math="f(x) = |x|" />.
