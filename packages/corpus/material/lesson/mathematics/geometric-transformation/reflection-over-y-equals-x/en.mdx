...
Reflection over the line <InlineMath math="y = x" /> <Highlight>is a geometric transformation that maps every point of an object to a new position</Highlight> with the line <InlineMath math="y = x" /> acting as a mirror.
...
<LineEquation
  ...
  data={[
    ...
    {
      points: [{ x: 4, y: 1, z: 0 }],
      color: "#059669", // Keeping color consistent for original and image as per user's id.mdx edit pattern
      showPoints: true,
...
All three problems reflect over the line <InlineMath math="y=x" />. The last one reflects a whole line, so its equation is transformed.
