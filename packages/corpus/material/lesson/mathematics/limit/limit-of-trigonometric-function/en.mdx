...
The special work begins when substitution produces an indeterminate form such as <InlineMath math="\frac{0}{0}" />. Near zero, one **fundamental sine limit** and familiar trigonometric identities let us rewrite those expressions into forms we can evaluate.

## Fundamental Theorem of Trigonometric Limits

The key result for trigonometric limits states that the ratio between the sine of an angle and the angle itself approaches one as the angle approaches zero.
...
> In this theorem, <InlineMath math="x" /> must be in **radians** rather than degrees. If using degrees, the result will be different.
