...
Two rules cover the sum and the product of functions with known limits. The subsections below state both rules and the condition each one requires.
...
A power or a root passes through the limit when the result stays inside the domain. The subsections below state that condition and apply it.
...
The examples below apply the rules to a polynomial and to a rational function. Each step names the rule it uses.
