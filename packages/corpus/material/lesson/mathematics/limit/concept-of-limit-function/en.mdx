export const metadata = {
    ...
    description: "Understand limits through value tables, one-sided behavior, the epsilon-delta definition, and fully worked examples.",
...
A **limit** asks what nearby output values are doing as the input approaches a chosen point. It does **not** begin by asking for the value at that point. This distinction lets us describe a removable hole, a jump, or a vertical asymptote without pretending that the function must be defined there.

Limits provide the language used to define continuity, derivatives, and definite integrals. We will first read the behavior from a value table, then connect that intuition to one-sided limits and the precise epsilon-delta definition.

## Approach Through Value Tables

To understand limits more concretely, see how function values change when the variable approaches a certain point. Suppose we have function <InlineMath math="f(x) = x + 2" /> and want to see what happens when <InlineMath math="x" /> approaches <InlineMath math="3" />.
