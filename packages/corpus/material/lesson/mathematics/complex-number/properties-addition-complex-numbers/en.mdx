...
Addition of complex numbers keeps the properties below, and each one is checked on the real part and the imaginary part separately.
...
Multiplying by a real scalar follows the same group of properties, so the same checks apply with one real factor.
