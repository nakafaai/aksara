export const metadata = {
  ...
  description: "Convert complex numbers among Cartesian, polar, and exponential forms with Euler's formula.",
...
Each complex number <InlineMath math="z = x + iy" /> can be placed at the point <InlineMath math="(x, y)" /> in the complex plane. The **position vector** from the origin <InlineMath math="(0, 0)" /> to that point represents the same number geometrically.
...
For <InlineMath math="z=0" />, the modulus is <InlineMath math="0" />, but no direction is defined. Therefore, zero has no argument and must be handled separately when using polar or exponential form.
...
Use the **modulus for length** and the quadrant to determine direction. This ensures that the chosen angle points to the actual complex number.
...
Find the **modulus and argument** of each number. For a point on an axis, read the direction directly instead of dividing by a zero real component.
...
2.  For <InlineMath math="z = -i" />:
    ...
    The point <InlineMath math="(0, -1)" /> lies on the negative imaginary axis, so its Principal Argument is <InlineMath math="\theta = -90^\circ" />.

    Polar form using the Principal Argument:

    <BlockMath math="z = 1(\cos (-90^\circ) + i \sin (-90^\circ))" />

    The coterminal angle <InlineMath math="270^\circ" /> gives the equivalent form
...
In Euler’s formula, the angle must be expressed in **radians**. Adding two pi to the angle makes one full turn and leaves the complex number unchanged.

Here, <InlineMath math="e \approx 2.71828\ldots" /> is Euler's number, the base of the natural logarithm.
...
Use **radians in the exponent**. The modulus stays unchanged because the new form only changes how we write the direction.
...
Convert the angle from **degrees to radians**, then keep the modulus in front of the exponential factor.
...
## Choosing a Suitable Form

<Highlight>Cartesian form is usually the clearest choice</Highlight> for addition and subtraction. Polar or exponential form makes multiplication, division, powers, and roots easier because moduli and arguments can be combined directly.
...
Two complex numbers <InlineMath math="z_1 = x_1 + iy_1" /> and <InlineMath math="z_2 = x_2 + iy_2" /> are **equal** if and only if their real parts are equal and their imaginary parts are equal.

<BlockMath math="\begin{aligned} &amp;z_1 = z_2 \\ &amp;\iff \\ &amp;x_1 = x_2 \text{ and } y_1 = y_2 \end{aligned}" />

### Equality Example

Compare **corresponding components**. A difference in just one component is enough to make the numbers unequal. Changing the order of the terms does not change the number.

| Pair | Real components | Imaginary components | Conclusion |
| --- | --- | --- | --- |
| <InlineMath math="3-2i" />, <InlineMath math="4+2i" /> | <InlineMath math="3\ne4" /> | <InlineMath math="-2\ne2" /> | Different |
| <InlineMath math="-1+i" />, <InlineMath math="i-1" /> | <InlineMath math="-1=-1" /> | <InlineMath math="1=1" /> | Equal |
...
2.  <InlineMath math="z_1 = 0 + 1i" /> and <InlineMath math="z_2 = 1 - 1i" />.
    ...
    The unequal imaginary parts show that <InlineMath math="z_1" /> is **different** from <InlineMath math="z_2" />.
...
Check the **components, quadrant, and representation** required by each question. The first three items are true or false. Next, convert representations and use component equations or the quadratic formula to solve the remaining problems.

1.  **True or False.** Every real number is a complex number.
2.  **True or False.** The three standard forms used here are Cartesian, exponential, and logarithmic.
...
5.  Find the real numbers <InlineMath math="x" /> and <InlineMath math="y" /> such that <InlineMath math="z_1 = x + 3i" /> and <InlineMath math="z_2 = 3 - yi" /> satisfy <InlineMath math="z_1 = z_2" />.
...
2.  **False.** The three standard forms used here are Cartesian, polar, and exponential. A logarithm is an operation applied to a complex number. It is not one of these three forms.
