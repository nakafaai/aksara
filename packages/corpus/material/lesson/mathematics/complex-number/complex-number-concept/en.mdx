...
Determine the real and imaginary parts of each complex number below. Two of these expressions hide their value inside a square root or a power of <InlineMath math="i" />, so simplify each one first and only then name the two parts.
