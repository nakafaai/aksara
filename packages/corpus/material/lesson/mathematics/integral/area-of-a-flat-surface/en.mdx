...
The integrand describes the vertical height of the region. If <InlineMath math="f(x) \ge 0" /> throughout <InlineMath math="[a,b]" />, the area is <InlineMath math="\int_{a}^{b} f(x) \, dx" />. If the graph crosses the axis, split the interval at its zeros or integrate <InlineMath math="|f(x)|" /> so that every part contributes positive area.
...
In this formula, <InlineMath math="F(x)" /> is [an antiderivative](/en/subjects/mathematics/integral/definition-of-indefinite-integral) of <InlineMath math="f(x)" />.
