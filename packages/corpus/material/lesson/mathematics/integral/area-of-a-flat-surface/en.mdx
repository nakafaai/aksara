...
## Basic Concepts of Area Under a Curve
...
**Definite integrals** handle such regions by accumulating the areas of increasingly narrow strips. This is the idea behind Riemann sums: partition the interval, approximate each strip with a rectangle, and examine the limiting sum.
...
Apply this concept to a concrete example. Suppose we want to calculate the area of the region bounded by the curve <InlineMath math="f(x) = x^2 - 4x" /> and the <InlineMath math="x" />-axis between <InlineMath math="x = 1" /> and <InlineMath math="x = 3" />.
