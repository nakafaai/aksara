...
**Definite integrals** handle such regions by <Highlight>accumulating the areas of increasingly narrow strips</Highlight>. A Riemann sum partitions the interval, approximates each strip with a rectangle, and takes the limit of their total area.
...
In this formula, <InlineMath math="F(x)" /> is an antiderivative of <InlineMath math="f(x)" />.
