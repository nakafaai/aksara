...
<Highlight>A function and its inverse undo each other</Highlight>. The order determines the set on which the identity applies: <InlineMath math="f\circ f^{-1}" /> is the identity on the range of <InlineMath math="f" />, while <InlineMath math="f^{-1}\circ f" /> is the identity on the domain of <InlineMath math="f" />.
