...
<Highlight>Dalam matematika, fungsi memetakan elemen dari satu himpunan (domain) ke himpunan lain (kodomain).</Highlight> Cara pemetaan ini bisa diklasifikasikan menjadi beberapa jenis berdasarkan bagaimana elemen domain dan kodomain dihubungkan. Tiga jenis utama adalah fungsi injektif, surjektif, dan bijektif.
...
Rumus <InlineMath math="q(x)=x^2" /> digunakan pada setiap pemetaan berikut. Tentukan apakah tiap fungsi bersifat injektif, surjektif, keduanya, atau bukan keduanya. Hanya kodomain yang berubah antarbutir, jadi bandingkan jawabannya untuk melihat bagaimana kodomain menentukan klasifikasinya.
