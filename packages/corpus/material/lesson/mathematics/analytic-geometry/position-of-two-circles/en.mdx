export const metadata = {
  ...
  description: "Classify two circles as intersecting, tangent, separate, or concentric from their center distance and radii.",
...
Two distinct circles may cross at two points, touch at one point, or have no common points. If both centers and radii are equal, the circles coincide and every point on their circumference is shared.

In analytic geometry, the centers and radii are enough to determine the relationship precisely. Compare the **distance between centers** with the sum and absolute difference of the radii.
...
- **Lower bound**: At <InlineMath math="|r_1 - r_2|>0" />, the smaller circle is internally tangent to the larger circle
...
One circle can also lie **inside another without touching**, even when their centers differ. In this diagram, the larger radius is <InlineMath math="4" />, the smaller radius is <InlineMath math="1" />, and the center distance is <InlineMath math="1" />.

<LineEquation
  title="Contained Circles without Contact"
  description="The centers differ and the circumferences do not meet."
  cameraPosition={[0, 0, 12]}
  showZAxis={false}
  data={[
    {
...
Since <InlineMath math="0<1<4-1" />, the smaller circle lies strictly inside the larger one. Their centers differ, so they are not concentric.
