...
Draw a circle and a straight line in the same plane. <Highlight>Exactly three positional relationships are possible</Highlight>.
...
Now calculate the **discriminant** from the coefficients <InlineMath math="A" />, <InlineMath math="B" />, and <InlineMath math="C" /> above:

<BlockMath math="\Delta = B^2 - 4AC" />

> For a **vertical line**, the form <InlineMath math="y=mx+c" /> does not apply. Substitute <InlineMath math="x=k" /> directly into the circle equation and solve the quadratic in <InlineMath math="y" />. For example, <InlineMath math="x=3" /> in <InlineMath math="x^2+y^2=9" /> gives <InlineMath math="9+y^2=9" />, so <InlineMath math="y=0" />. The line is tangent at <InlineMath math="(3,0)" />.
...
| Discriminant | Real roots | Line position |
| --- | --- | --- |
| <InlineMath math="\Delta>0" /> | Two distinct roots | **Two intersections** |
| <InlineMath math="\Delta=0" /> | One repeated root | **Tangent at one point** |
| <InlineMath math="\Delta<0" /> | None | **No intersection** |
...
To **find the intersection points**, use the quadratic formula and substitute each horizontal coordinate back into the line equation:
...
Pair the upper signs together and the lower signs together. Each coordinate pair satisfies both the line equation and the circle equation.
...
The interpretation is the same as before: <InlineMath math="\Delta > 0" /> means the line cuts the circle at two points, <InlineMath math="\Delta = 0" /> means it touches at one point, and <InlineMath math="\Delta < 0" /> means no intersection.
