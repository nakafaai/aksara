...
The line can **intersect the circle at two points**, **be tangent at one point**, or **miss the circle entirely**. A pencil laid across a ring gives a simple picture: it can cross the ring, touch only its rim, or remain outside it.

Analytic geometry turns this picture into a reliable calculation. The same method can test whether a planned road crosses a circular protected area or whether a path reaches a defined zone.
...
To determine the case algebraically, substitute the line equation into the circle equation. The result is a quadratic equation whose **discriminant** counts the real intersection points.
