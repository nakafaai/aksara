...
A chord's length and its distance from the centre move together. The results below state that relation and the equal-length case.
...
A chord and its central angle form an isosceles triangle with two radii. The formula below comes from splitting that triangle in half.
