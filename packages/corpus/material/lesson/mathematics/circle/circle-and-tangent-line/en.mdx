...
A tangent touches the circle at exactly one point, and the radius to that point is perpendicular to the tangent. The subsections below write the tangent equation for a point on the circle and for a point outside it.
...
Two circles can share tangent lines that cross between them or pass outside. The subsections below build each kind and give its length.
...
This positive-length external tangent segment exists only when <InlineMath math="d>|r_1-r_2|" />. Equality describes internal tangency, where the segment between the two contact points collapses to length zero.
...
The examples below build tangent equations from different starting information. Work through the substitution in each case.
...
5.  **Tangent line through a point on the circle**
