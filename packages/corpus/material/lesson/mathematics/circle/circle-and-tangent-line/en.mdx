...
A tangent <Highlight>touches the circle at exactly one point</Highlight>, and the radius to that point is perpendicular to the tangent. The subsections below write the tangent equation for a point on the circle and for a point outside it.
...
Two circles can share tangent lines that cross between them or pass outside. The subsections below build each kind and give its length. Whether a common tangent exists at all depends on how far apart the two circles are.
