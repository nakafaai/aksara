...
Expected value is the probability-weighted average of a random variable. It describes a long-run mean rather than a guaranteed result in one set. An expected value can also be noninteger even when the random variable itself only takes integer values.
