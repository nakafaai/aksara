...
1. **Find All Complex Roots:** Find all <InlineMath math="n" /> complex roots (zeros) of <InlineMath math="P(x) = 0" />. This might involve:
   ...
   - Using the [Rational Zero Theorem](/en/subjects/mathematics/polynomial/rational-zero) to find rational roots.
...
1. **Find Roots:** Try to factor <InlineMath math="P(x)" /> first.
...
If <InlineMath math="a+bi" /> with <InlineMath math="b\neq0" /> is a root of a polynomial with real coefficients, then <InlineMath math="a-bi" /> is also a root. The two roots form a conjugate pair, so such a polynomial cannot have exactly one non-real root.
