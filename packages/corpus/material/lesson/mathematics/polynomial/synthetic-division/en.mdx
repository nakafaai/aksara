...
<Highlight>Polynomial long division finds the quotient and remainder by repeating the same divide, multiply, subtract, and bring-down pattern used in integer long division.</Highlight> Column alignment is what makes the pattern work, because only like terms may be subtracted from each other.
