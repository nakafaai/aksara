...
The Factor Theorem turns a verified zero into a linear factor. Repeating that process reduces the degree until the remaining polynomial can be factored in the chosen number system.
...
Let <InlineMath math="P(x) = x^3 - 2x^2 - 21x - 18" />. Show that <InlineMath math="P(-1) = 0" />, then use <InlineMath math="P(-1) = 0" /> to factor <InlineMath math="P(x)" /> completely. After Horner's method gives the first factor, divide again until the quotient is quadratic, then factor that quotient.
