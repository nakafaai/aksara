...
Polynomial addition and subtraction combine only **like terms**, just as they do in other algebraic expressions.
...
Here are example sketches of the graphs resulting from the sum <InlineMath math="f(x) + g(x)" /> and difference <InlineMath math="f(x) - g(x)" />, obtained by vertically adding or subtracting the <InlineMath math="y" />-values for each <InlineMath math="x" />.
...
Find <InlineMath math="A(x)+B(x)" /> and <InlineMath math="A(x)-B(x)" />.
