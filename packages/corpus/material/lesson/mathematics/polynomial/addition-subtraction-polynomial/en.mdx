...
In the same way, you can sketch the graphs of <InlineMath math="f(x) + h(x), \space g(x) + h(x), \space f(x) - h(x), \space \text{and} \space g(x) - h(x)" />. The key is to add or subtract the heights (<InlineMath math="y" />-values) of the original graphs at each corresponding <InlineMath math="x" />-value.
