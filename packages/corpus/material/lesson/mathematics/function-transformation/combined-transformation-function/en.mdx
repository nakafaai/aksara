...
When transformations are combined, the following rules determine the result:

- **Order-sensitive**: Some transformations do not commute, so changing their order changes the result. Independent horizontal and vertical transformations, however, can be applied in either order
- **Operations can be combined**: Consecutive translations in the same direction add their offsets, while consecutive dilations multiply their scale factors
