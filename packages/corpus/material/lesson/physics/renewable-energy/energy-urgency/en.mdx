...
**Urgency** means the reason a problem needs attention now rather than later. Energy problems concern the amount of electricity and whether it is available when needed, reaches the right places, stays reliable, remains affordable, and avoids excessive environmental damage.
...
Think of a school canteen. The canteen can serve many students if the food, cashier, and queue system are ready. If all students come at the same time while the food and cashier are not enough, the system cannot meet demand at that moment. An energy system has the same kind of timing problem across its sources, power plants, grids, storage, and usage habits.
...
These questions make the energy issue more concrete by showing which part is urgent and why.
...
The urgent task is to make the needed energy available to users in a way that suits the local environment and community.
