...
So the most useful question is not only "is this source renewable?". A better question is "is this source available enough at the location and time when energy is needed?". For managed resources such as biomass and water, the check must also include ecosystem limits and the rate at which the local resource can recover.
