...
One detail often causes confusion: the SI base unit for mass is <InlineMath math="\text{kg}" /> rather than <InlineMath math="\text{g}" />. So when mass is written in grams, convert it to kilograms if you want the full SI base unit form.
...
A prefix is attached to a base unit. For example, <InlineMath math="\text{km}" /> means kilo-meter rather than a separate unit unrelated to the meter.
