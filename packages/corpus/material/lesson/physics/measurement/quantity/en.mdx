...
The numerical value <InlineMath math="0.60" /> tells us *how much*. The unit <InlineMath math="\text{m}" /> tells us *which reference it is compared with*. The quantity is the property of an object or event being measured, such as length, mass, time, temperature, or speed. A complete measurement result can also carry uncertainty and other relevant information.
...
Once a unit is built by combining other units, its quantity is derived rather than base. Compare the dimensions on both sides of a formula to test whether its units are compatible.

The terminology follows the definition of quantity value in the [JCGM VIM](https://jcgm.bipm.org/vim/en/1.19.html).
