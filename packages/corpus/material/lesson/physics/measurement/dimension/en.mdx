...
export const metadata = {
  title: "Dimensions",
  description:
    "Physical dimensions express derived quantities as powers of base quantities and test whether formulas are dimensionally consistent without using numerical values.",
...
Length, diameter, radius, and distance all have the length dimension <InlineMath math="[\mathrm{L}]" />.
...
The visual below focuses only on the length dimension. Switch from length to area and then volume. Notice how one, two, and three **length factors** give dimensions <InlineMath math="[\mathrm{L}]" />, <InlineMath math="[\mathrm{L}]^2" />, and <InlineMath math="[\mathrm{L}]^3" />.

<DimensionLab
  title={<>From Length to Area and Volume</>}
  description={
    <>
...
The International System of Units or SI uses <InlineMath math="7" /> base quantities. The dimension of a derived quantity is written as a **product of powers** of the seven base dimensions.
...
Dimensions can help check whether a formula form could be correct. Every term being added <Highlight>must have the same dimension</Highlight>.
...
Both terms on the right have dimension <InlineMath math="[\mathrm{L}]" />, so the formula passes the dimensional check. Passing this check cannot prove that a formula is correct. The check can still catch impossible formulas quickly.

<InlineMath math="s+t" /> is invalid here because the dimensions differ: <InlineMath math="[\mathrm L]\ne[\mathrm T]" />.
...
The limit of the check becomes clear if we omit <InlineMath math="\frac{1}{2}" /> from the displacement formula. The expression <InlineMath math="s=v_0t+at^2" /> still has length dimensions on both sides, but gives the wrong displacement for constant nonzero acceleration. **Dimensions do not determine numerical factors** such as <InlineMath math="\frac{1}{2}" />.
