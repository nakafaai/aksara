...
Significant figures are <Highlight>the digits retained to communicate the resolution</Highlight> of a measured value. Write the value only to the decimal place supported by the instrument. The last reported digit shows the limit of the reading and can change when the value is rounded.
...
With <InlineMath math="1" /> significant figure:

<BlockMath math="1000\text{ m}=1\times10^3\text{ m}" />

With <InlineMath math="4" /> significant figures:
...
For addition and subtraction, round by decimal position. For multiplication and division, round by the fewest significant figures among the measured values. [Round-to-even and one-step rounding](https://www.nist.gov/document/2023-nist-hb130-uniform-packaging-and-labeling-regulation) prevent repeated rounding from biasing the result.
