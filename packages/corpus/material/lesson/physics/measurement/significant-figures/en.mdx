...
In scientific notation, a power such as <InlineMath math="10^{-5}" /> only gives the scale rather than a new significant figure.
...
For multiplication and division, the classroom significant-figure rule uses the smallest number of significant figures among the measured data. It is a reporting shortcut rather than a replacement for uncertainty propagation when measured uncertainties are available.
