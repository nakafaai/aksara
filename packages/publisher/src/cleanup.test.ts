...
const runCleanup = Effect.fn("CleanupContentReleaseTest.run")(
  (input: unknown, cleanup: ReturnType<typeof receiptSequence>) =>
    cleanupContentRelease(input).pipe(
      Effect.provideService(PublicationTarget, makeTarget(cleanup))
    )
);

describe("cleanupContentRelease", () => {
  it.effect("loops over cumulative progress until cleanup completes", () =>
    Effect.gen(function* () {
      const cleanup = receiptSequence([progress, complete]);
      const result = yield* runCleanup({ releaseId }, cleanup);
  ...
  it.effect("returns resumable evidence after one bounded call budget", () =>
    Effect.gen(function* () {
  ...
  it.effect(
    "returns a typed defer without waiting or requesting another page",
    () =>
      Effect.gen(function* () {
        const retryAt = 1_800_000_000_000;
        const cleanup = receiptSequence([{ ...progress, retryAt }]);
        const error = yield* runCleanup({ releaseId }, cleanup).pipe(
          Effect.flip
        );
  ...
  it.effect("rejects malformed input before target cleanup", () =>
    Effect.gen(function* () {
      const cleanup = receiptSequence([complete]);
      const error = yield* runCleanup(
        { cursor: null, releaseId },
        cleanup
      ).pipe(Effect.flip);
  ...
  it.effect("rejects foreign identities and decreasing cumulative counts", () =>
    Effect.gen(function* () {
      ...
      const errors = yield* Effect.forEach(cases, (cleanup) =>
