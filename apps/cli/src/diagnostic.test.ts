...
describe("document failure diagnostics", () => {
  it("explains metadata, byte, renderer, module, and MDX failures", async () => {
    const failures = await Promise.all([
      ...
      compileFailure(withMetadata('import value from "./value"\n\n{value}')),
  ...
  it("bounds repeated policy failures and preserves identifier-free rules", async () => {
    const repeated = Array.from({ length: 9 }, () => "{process.env}").join(
      "\n"
    );
    const [many, identifierFree] = await Promise.all([
