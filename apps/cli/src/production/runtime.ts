import { ReleasePolicyClosureError } from "@nakafa/aksara-contracts/release/policy";
...
export const selectTryoutRuntimeRefresh = Effect.fn(
  ...
  if (snapshotId !== null && input.bundle === null && !selectsTryout) {
    return yield* new ReleasePolicyClosureError({
      actual: "missing",
      expected: "runtime-refresh",
      family: "tryout",
      field: "scope",
    });
  }
  ...
  return refresh
    ? ({
        kind: "refresh",
        snapshot: input.bundle?.payload.snapshot ?? null,
