...
/** Creates one npm signature audit fixture. */
function audit(
  options: {
...
/** Creates one deterministic bundle-verifier test adapter. */
function verifier(
  payload: string,
...
describe("npm provenance", () => {
  ...
  it.effect("rejects malformed audit documents", () =>
    Effect.gen(function* () {
      const result = yield* verifyProvenance("{}", EXPECTATION).pipe(
        Effect.provide(verifier(statement())),
        Effect.result
      );
